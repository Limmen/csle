...
def test_start_ossec_manager(container_setup) -> None:
    ...
    try:
        ...
        logging.info(f"Starting ossec manager in container: {container_setup.id} "
        ...
        logging.info(f"Verifying that ossec manager is running in container: {container_setup.id} "
                     f"with image: {container_setup.image.tags}")
