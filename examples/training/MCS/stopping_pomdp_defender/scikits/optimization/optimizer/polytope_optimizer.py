    ...
    def iterate(self):
        ...
        if value < self.state["polytope_values"][-2]:
            if value > self.state["polytope_values"][0]:
                ...
                point_expansion, value_expansion = self.get_value(
                    mean, discarded_point, -2
                )
                if value_expansion < value:
                    self.optimal_point = numpy.vstack(
                        (self.optimal_point[:-1], point_expansion)
                    )
                else:
                    self.optimal_point = numpy.vstack((self.optimal_point[:-1], point))
        else:
            if value < self.state["polytope_values"][-1]:
                point_contraction, value_contraction = self.get_value(
                    mean, discarded_point, -0.5
                )
                if value_contraction < value:
                    self.optimal_point = numpy.vstack(
                        (self.optimal_point[:-1], point_contraction)
                    )
                else:
                    self.optimal_point = (
                        self.optimal_point + self.optimal_point[0]
                    ) / 2
            else:
                point_contraction, value_contraction = self.get_value(
                    mean, discarded_point, 0.5
