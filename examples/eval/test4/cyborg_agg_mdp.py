...
class Cage2AggregateMDP:
    ...
    @staticmethod
    def get_aggregate_control(mu, aggregate_state):
        """
    ...
    def get_aggregate_state(s: CyborgWrapperState, state_to_id):
        ...
        target = Cage2AggregateMDP.red_target_to_aggregate_target(s.red_agent_target)
        red_state = s.red_agent_state
        decoy_state_str = ",".join(list(map(lambda x: str(x), decoy_state)))
        state_str = f"{red_state},{target},{decoy_state_str}"
    ...
    def red_target_to_aggregate_target(target):
        ...
        """
        red_target_to_target_idx = {0: 4, 1: 0, 2: 1, 3: 2, 4: 0, 5: 0, 6: 0, 7: 7, 8: 4, 9: 3, 10: 4, 11: 5, 12: 6}
    ...
    def vi(X, U, gamma, epsilon, verbose, state_to_id, id_to_state):
        ...
        while True:
            ...
            if verbose:
                ...
                ssx = [state_to_id[f"0,3,0,0,0,0,0,0,0"], state_to_id[f"1,3,0,0,0,0,0,0,0"],
                       state_to_id[f"1,3,0,0,0,0,0,0,0"], state_to_id[f"2,3,0,0,0,0,0,0,0"],
                       state_to_id[f"3,1,0,0,0,0,0,0,0"], state_to_id[f"4,1,0,0,0,0,0,0,0"],
                       state_to_id[f"5,1,0,0,0,0,0,0,0"], state_to_id[f"6,2,0,0,0,0,0,0,0"],
                       state_to_id[f"7,2,0,0,0,0,0,0,0"], state_to_id[f"8,2,0,0,0,0,0,0,0"],
                       state_to_id[f"9,2,0,0,0,0,0,0,0"], state_to_id[f"10,7,0,0,0,0,0,0,0"],
                       state_to_id[f"11,7,0,0,0,0,0,0,0"], state_to_id[f"12,7,0,0,0,0,0,0,0"],
                       state_to_id[f"13,7,0,0,0,0,0,0,0"], state_to_id[f"14,7,0,0,0,0,0,0,0"]]
                for sx in ssx:
