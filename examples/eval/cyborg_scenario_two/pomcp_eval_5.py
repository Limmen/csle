    ...
    for i in range(num_evaluations):
        ...
        while t < max_horizon:
            ...
            if action_sequence[-1] == 1 or action_sequence[-1] == 0:
                target = 6
            # if t < 5:
            #     action_space_reduction.append((145.0-10)/145.0)
            # else:
            action_space_reduction.append((145.0-len(pomcp.tree.root.children) -3)/145.0)
            pomcp.update_tree_with_new_samples(action_sequence=action_sequence, observation=obs_id, t=t + 1)
            R += r
            t += 1
            # print(action_space_reduction)
            Logger.__call__().get_logger().info(f"[POMCP] t: {t}, a: {action}, r: {r}, o: {obs_id}, "
                                                f"s_prime: {s_prime},"
                                                f", action sequence: {action_sequence}, R: {R}")
        action_space_reductions.append(action_space_reduction)
        means = []
        stds = []
        for k in range(len(action_space_reductions[0])):
            vals = []
            for i in range(len(action_space_reductions)):
                vals.append(action_space_reductions[i][k])
            mean = np.mean(vals)
            std = np.std(vals)
            means.append(mean)
            stds.append(std)
        ...
        # with io.open(f"/Users/kim/p_orig_meander_0_05s_T_100_seed_{seed}.json", 'w', encoding='utf-8') as f:
        #      f.write(json_str)
