    ...
    for i in range(num_evaluations):
        ...
        while t < max_horizon:
            ...
            if "Exploit" in str(red_action):
                ...
                host= csle_cyborg_env.get_ip_to_host_mapping()[str(ip)]
                host_id = csle_cyborg_env.cyborg_hostnames.index(host)
                # if obs[host_id][0] == 2 and s[host_id][2] == 0:
                #     print(f"Exploit failed but activity detected, decoy state: {s[host_id][3]}")
                    # print(csle_cyborg_env.get_actions_table())
                    # print(csle_cyborg_env.get_true_table())
                    # print(csle_cyborg_env.get_table())
                # if blue_targets[-1] == red_targets[-1] and s[host_id][2] > 0 and states[-2][host_id][2] > 0 \
                #         and blue_types[-1] in csle_cyborg_env.decoy_action_types \
                #         and states[-2][host_id][3] < len(csle_cyborg_env.decoy_actions_per_host[host_id]):
                if blue_targets[-1] == red_targets[-1] and s[host_id][2] > 0\
                        and blue_types[-1] in csle_cyborg_env.decoy_action_types \
                        and states[-2][host_id][3] < len(csle_cyborg_env.decoy_actions_per_host[host_id]):
                # if s[host_id][2] > 0:
                    print(f"Decoy at the same time as successful exploit and already compromised, "
                          f"obs: {obs[host_id]}, host: {host_id}, previous decoy state: {states[-2][host_id][3]}, "
                          f"previous compromised state: {states[-2][host_id][2]}")
                    if obs[host_id][0] == 0 or host_id == 11:
                        print(s[host_id])
                        print(states[-2][host_id])
                        print(BlueAgentActionType(blue_types[-1]))
                        print(csle_cyborg_env.get_actions_table())
                        print(csle_cyborg_env.get_true_table())
                        print(csle_cyborg_env.get_table())
