...
def empirical_discrepancy(feedback_probabilities: List[Any], theta: int, true_theta: int, Theta: List[int]) -> float:
    ...
    for tau in range(len(feedback_probabilities[true_theta_idx])):
        if feedback_probabilities[conjecture_idx][tau] > 0:
            quotient = feedback_probabilities[true_theta_idx][tau] / feedback_probabilities[conjecture_idx][tau]
            if quotient == 0:
                Z += 10
            else:
...
def online_play(T: int, b1: List[float], pi_d_1: float, pi_a_1: npt.NDArray[Any], gamma: float, ell_d: int, ell_a: int,
    ...
    for t in range(2, T):
        ...
        if t % 5 == 0:
            ...
            with io.open(f"/home/kim/model_estimator_seed_{seed}_{len(Theta)}.json", 'w', encoding='utf-8') as f:
                f.write(json_str)
        # true_theta = t
...
if __name__ == '__main__':
    ...
    Theta = list(range(2, 76))
    # for i in range(2,101):
    #     print(f"{i} {data_dict['rate_means'][i]}")
    # Theta = [2, 10]
