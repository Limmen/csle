...
def update_model(attacker_model, target, access, red_action, decoy_state, target_access):
    if target not in attacker_model["access_given_target_redaction_decoystate_and_access"]:
        attacker_model["access_given_target_redaction_decoystate_and_access"][target] = {}
    if red_action not in attacker_model["access_given_target_redaction_decoystate_and_access"][target]:
        attacker_model["access_given_target_redaction_decoystate_and_access"][target][red_action] = {}
    if decoy_state not in attacker_model["access_given_target_redaction_decoystate_and_access"][target][red_action]:
        attacker_model["access_given_target_redaction_decoystate_and_access"][target][red_action][decoy_state] = {}
    if access not in attacker_model["access_given_target_redaction_decoystate_and_access"][target][red_action][decoy_state]:
        attacker_model["access_given_target_redaction_decoystate_and_access"][target][red_action][decoy_state][access] = []
    attacker_model["access_given_target_redaction_decoystate_and_access"][target][red_action][decoy_state][access].append(target_access)
...
if __name__ == '__main__':
    ...
    counts = {}
    counts["access_given_target_redaction_decoystate_and_access"] = {}
    num_episodes = 1000
        ...
        while not done:
            ...
            if position < len(state_vector):
                position_access_state = get_access_state(host_state_vector=state_vector[position],
                                                       access_state_lookup=access_state_lookup)
            counts = update_model(attacker_model=counts, target=target_idx,
                                  target_access=target_access_state, red_action=red_action_type,
                                  decoy_state=target_decoy_state, access=position_access_state)

    probability_model = {}
    probability_model["access_given_target_redaction_decoystate_and_access"] = {}
    for k,v in counts["access_given_target_redaction_decoystate_and_access"].items():
        probability_model["access_given_target_redaction_decoystate_and_access"][int(k)] = {}
        for k2,v2 in v.items():
            probability_model["access_given_target_redaction_decoystate_and_access"][int(k)][int(k2)] = {}
            for k3,v3 in v2.items():
                probability_model["access_given_target_redaction_decoystate_and_access"][int(k)][int(k2)][int(k3)] = {}
                for k4,v4 in v3.items():
                    probability_model["access_given_target_redaction_decoystate_and_access"][int(k)][int(k2)][int(k3)][int(k4)] = {}
                    distribution = convert_samples_to_distribution(samples=counts["access_given_target_redaction_decoystate_and_access"][k][k2][k3][k4])
                    for k5,v5 in distribution.items():
                        probability_model["access_given_target_redaction_decoystate_and_access"][int(k)][int(k2)][int(k3)][int(k4)][int(k5)] = float(v5)
    print(probability_model)
    json_str = json.dumps(probability_model, indent=4, sort_keys=True)
    with io.open(f"/home/kim/attacker_model2.json", 'w', encoding='utf-8') as f:
        f.write(json_str)
