...
if __name__ == '__main__':
    ...
    env = AptGameEnv(config=input_config)
    p_intrusion = 0.5
    pi2 = []
    for s in S:
    ...
    ell = 1
    start = time.time()
    RolloutUtil.monte_carlo_defender_rollout(
        alpha=alpha, pi2=pi2, config=env.config, num_samples=num_samples, horizon=horizon, ell=ell, b=b)
    end = time.time()
    print(end - start)
