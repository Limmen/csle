...
#from gym_cgc_bta.agents.policy_gradient.openai_baselines.ppo.ppo import PPO

class PPOBaselineAgent(PolicyGradientAgent):
    """
    An agent for the cgc-bta env that uses the PPO Policy Gradient algorithm from OpenAI stable baselines
    """
    ...
    def __init__(self, env: PyCRPwCrackEnv, config: PolicyGradientAgentConfig):
        ...
        """
        super(PPOBaselineAgent, self).__init__(env, config)
    ...
    def train(self) -> ExperimentResult:
        ...
        if self.config.load_path is not None:
            PPO.load(self.config.load_path, policy, pg_agent_config=self.config)
