...
class PyCrPwCrackLevel5Base:
    """
    Base configuration of level 5 of the PyCrPwCrack environment. (Mainly used when running in simulation mode
    ...
    def network_conf(generate: bool = False) -> NetworkConfig:
        ...
        agent_reachable = set()
        if not generate:
            nodes = PyCrPwCrackLevel5Base.nodes()
            adj_matrix = PyCrPwCrackLevel5Base.adj_matrix()
            agent_reachable = PyCrPwCrackLevel5Base.agent_reachable()
        network_conf = NetworkConfig(subnet_mask=PyCrPwCrackLevel5Base.subnet_mask(),
                                     nodes=nodes,
    ...
    def env_config(network_conf : NetworkConfig, action_conf: ActionConfig, cluster_conf: ClusterConfig,
        ...
        env_config = EnvConfig(network_conf=network_conf, action_conf=action_conf, num_ports=10, num_vuln=10,
                               num_sh=3, num_nodes = PyCrPwCrackLevel5Base.num_nodes(), render_config=render_conf,
                               env_mode=EnvMode.SIMULATION,
                               cluster_config=cluster_conf,
                               simulate_detection=True, detection_reward=10, base_detection_p=0.05,
                               hacker_ip=PyCrPwCrackLevel5Base.hacker_ip(), state_type=StateType.BASE,
                               router_ip=PyCrPwCrackLevel5Base.router_ip())
