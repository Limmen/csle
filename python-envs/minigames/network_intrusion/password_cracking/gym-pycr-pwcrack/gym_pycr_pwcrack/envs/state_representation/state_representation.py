...
class StateRepresentation:
    ...
    @staticmethod
    def compact_representation(num_machines: int, num_m_features: int, num_ports: int, obs_state: ObservationState,
                            vuln_lookup: dict, service_lookup: dict, os_lookup: dict) \
            -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        :param num_machines: max number of machines in the obs
        :param num_m_features: num features per machine
        :param num_ports: num ports
        :param obs_state: current observation state to turn into a numeratical representation
        :param vuln_lookup: lookup dict for converting categorical vuln into numerical
        :param service_lookup: lookup dict for converting categorical service into numerical
        ...
        for i in range(num_machines):
            ...
            if len(obs_state.machines) > i:
                ...
                # Num Open Ports
                machines_obs[i][1] = len(obs_state.machines[i].ports)

                # Num Vulnerabilities
                machines_obs[i][2] = len(obs_state.machines[i].cve_vulns)

                # Total CVSS score
                total_cvss = sum(list(map(lambda x: x.cvss, obs_state.machines[i].cve_vulns)))
                machines_obs[i][3] = total_cvss

                # Shell Access
                machines_obs[i][4] = int(obs_state.machines[i].shell_access)

                # Logged in
                machines_obs[i][5] = int(obs_state.machines[i].logged_in)

                # Root access
                machines_obs[i][6] = int(obs_state.machines[i].root)

                # Flag pts
                flag_pts_score = sum([f.score for f in obs_state.machines[i].flags_found])
                machines_obs[i][7] = int(flag_pts_score)
