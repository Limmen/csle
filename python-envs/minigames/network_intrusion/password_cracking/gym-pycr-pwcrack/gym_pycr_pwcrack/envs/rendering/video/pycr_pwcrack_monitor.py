...
import imageio
from gym_pycr_pwcrack.envs.rendering.video import pycr_pwcrack_stats_recorder
from gym_pycr_pwcrack.envs.rendering.video import pycr_pwcrack_video_recorder
...
class PycrPwCrackMonitor(Wrapper):
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(directory, ', '.join(training_manifests[:5])))
        ...
        self.stats_recorder = pycr_pwcrack_stats_recorder.StatsRecorder(
    ...
    def close(self):
        ...
        super(PycrPwCrackMonitor, self).close()
