...
class PyCrCTFLevel6Base:
    """
    Base configuration of level 6 of the PyCrCTF environment. (Mainly used when running in simulation mode
    ...
    def network_conf(generate: bool = False) -> NetworkConfig:
        ...
        agent_reachable = set()
        if not generate:
            nodes = PyCrCTFLevel6Base.nodes()
            adj_matrix = PyCrCTFLevel6Base.adj_matrix()
            agent_reachable = PyCrCTFLevel6Base.agent_reachable()
        network_conf = NetworkConfig(subnet_mask=PyCrCTFLevel6Base.subnet_mask(),
                                     nodes=nodes,
                                     adj_matrix=adj_matrix,
                                     flags_lookup = PyCrCTFLevel6Base.flags_lookup(),
    ...
    def env_config(network_conf : NetworkConfig, attacker_action_conf: AttackerActionConfig,
        ...
        env_config = EnvConfig(network_conf=network_conf, attacker_action_conf=attacker_action_conf,
                               ...
                               attacker_num_sh_obs=3, num_nodes = PyCrCTFLevel6Base.num_nodes(), render_config=render_conf,
                               env_mode=EnvMode.SIMULATION,
                               emulation_config=emulation_config,
                               simulate_detection=True, detection_reward=10, base_detection_p=0.05,
                               hacker_ip=PyCrCTFLevel6Base.hacker_ip(), state_type=StateType.BASE,
                               router_ip=PyCrCTFLevel6Base.router_ip())
