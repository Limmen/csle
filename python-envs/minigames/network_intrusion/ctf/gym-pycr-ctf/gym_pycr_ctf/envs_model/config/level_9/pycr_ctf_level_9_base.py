...
class PyCrCTFLevel9Base:
    """
    Base configuration of level 9 of the PyCrCTF environment. (Mainly used when running in simulation mode
    ...
    def flags_lookup() -> str:
        ...
        """
        flags_lookup = {}
        flags_lookup[("172.18.9.2", "/tmp/flag2")] = Flag(name="flag2", path="/tmp", id=2, requires_root=False, score=1)
        flags_lookup[("172.18.9.3", "/root/flag1")] = Flag(name="flag1", path="/root", id=1, requires_root=True,
                                                           score=1)
        flags_lookup[("172.18.9.79", "/tmp/flag3")] = Flag(name="flag3", path="/tmp", id=3, requires_root=False,
                                                           score=1)
        flags_lookup[("172.18.9.54", "/tmp/flag4")] = Flag(name="flag4", path="/tmp", id=4, requires_root=False,
                                                           score=1)
        flags_lookup[("172.18.9.61", "/root/flag5")] = Flag(name="flag5", path="/root", id=5, requires_root=True,
                                                           score=1)
        flags_lookup[("172.18.9.7", "/tmp/flag6")] = Flag(name="flag6", path="/tmp", id=6, requires_root=False,
                                                            score=1)
        return flags_lookup
    ...
    def network_conf(generate: bool = False) -> NetworkConfig:
        ...
        agent_reachable = set()
        if not generate:
            nodes = PyCrCTFLevel9Base.nodes()
            adj_matrix = PyCrCTFLevel9Base.adj_matrix()
            agent_reachable = PyCrCTFLevel9Base.agent_reachable()
        network_conf = NetworkConfig(subnet_mask=PyCrCTFLevel9Base.subnet_mask(),
                                     nodes=nodes,
                                     adj_matrix=adj_matrix,
                                     flags_lookup = PyCrCTFLevel9Base.flags_lookup(),
                                     agent_reachable=agent_reachable,
                                     vulnerable_nodes=set(["172.18.9.3", "172.18.9.79", "172.18.9.2",
                                                           "172.18.9.54", "172.18.9.74", "172.18.9.61",
                                                           "172.18.9.62", "172.18.9.7"])
                                     )
        return network_conf

    @staticmethod
    def agent_reachable() -> set():
        reachable = set(["172.18.9.10", "172.18.9.2", "172.18.9.3", "172.18.9.21", "172.18.9.79"])
        return reachable
    ...
    def emulation_config() -> EmulationConfig:
        ...
        """
        emulation_config = EmulationConfig(server_ip="172.31.212.91", agent_ip="172.18.9.191",
    ...
    def env_config(network_conf : NetworkConfig, attacker_action_conf: AttackerActionConfig,
        ...
        env_config = EnvConfig(network_conf=network_conf, attacker_action_conf=attacker_action_conf,
                               ...
                               attacker_num_sh_obs=3, num_nodes = PyCrCTFLevel9Base.num_nodes(), render_config=render_conf,
                               env_mode=EnvMode.SIMULATION,
                               emulation_config=emulation_config,
                               simulate_detection=True, detection_reward=10, base_detection_p=0.05,
                               hacker_ip=PyCrCTFLevel9Base.hacker_ip(), state_type=StateType.BASE,
