...
class PyCrCTFLevel5V2:
    """
    V2 configuration of level 5 of the PyCrCTF environment.
    """
    ...
    def attacker_actions_conf(num_nodes: int, subnet_mask: str, hacker_ip: str = None) -> AttackerActionConfig:
        ...
        # Subnet actions
        attack_actions.append(AttackerNMAPActions.TCP_SYN_STEALTH_SCAN(index=num_nodes + 1, ip=subnet_mask,
                                                                subnet=True))
    ...
    def env_config(network_conf : NetworkConfig,
        ...
        env_config = EnvConfig(network_conf=network_conf, attacker_action_conf=attacker_action_conf, attacker_num_ports_obs=10, attacker_num_vuln_obs=10,
                               attacker_num_sh_obs=3, num_nodes = PyCrCTFLevel5Base.num_nodes(),
                               render_config=render_conf, env_mode=EnvMode.emulation,
                               emulation_config=emulation_config,
                               simulate_detection=True, detection_reward=10, base_detection_p=0.05,
                               hacker_ip=PyCrCTFLevel5Base.hacker_ip(), state_type=StateType.ESSENTIAL,
