...
def plot():
    ...
    plot_stopping_num_alerts(model, env)

def plot_stopping_num_alerts(model, env):
    ...
    fig.savefig("test_model" + ".png", format="png", dpi=600)
    fig.savefig("test_model" + ".pdf", format='pdf', dpi=600, bbox_inches='tight', transparent=True)
    # plt.close(fig)
...
def plot_value_logged_in(model, env):
    ...
    ax.set_title(r"$\pi(stop | w_a, s_a)$", fontsize=16)
