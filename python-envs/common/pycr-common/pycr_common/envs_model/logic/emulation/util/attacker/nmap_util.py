...
class NmapUtil:
    ...
    @staticmethod
    def check_nmap_action_cache(a: AttackerAction, env_config: PyCREnvConfig, conn=None, dir: str = None,
                                machine_ip: str = None):
        """
    ...
    @staticmethod
    def check_nmap_action_cache_interactive(a: AttackerAction, env_config: PyCREnvConfig):
        """
        ...
        # Clear channel
        if env_config.emulation_config.agent_channel.recv_ready():
            output = env_config.emulation_config.agent_channel.recv(5000)

        # List cache
        env_config.emulation_config.agent_channel.send(constants.COMMANDS.LIST_CACHE
                                                       + env_config.nmap_cache_dir + "\n")
        result_str = EmulationUtil.read_result_interactive(env_config=env_config)
    ...
    @staticmethod
    def parse_nmap_scan(file_name: str, env_config: PyCREnvConfig, conn=None, dir: str = None) -> ET.Element:
        """
    ...
    @staticmethod
    def parse_nmap_scan_interactive(file_name: str, env_config: PyCREnvConfig) -> ET.Element:
        """
        ...
        :return: the parsed XML file
        """
        env_config.emulation_config.agent_channel.send("cat " + env_config.nmap_cache_dir + file_name + "\n")
        while not env_config.emulation_config.agent_channel.recv_ready():
            time.sleep(env_config.shell_read_wait)
        output = env_config.emulation_config.agent_channel.recv(env_config.max_nmap_command_output_size)
    ...
    @staticmethod
    def merge_nmap_scan_result_with_state(scan_result: NmapScanResult, s: EnvState, a: AttackerAction, env_config: PyCREnvConfig) \
    ...
    @staticmethod
    def nmap_scan_action_helper(s: EnvState, a: AttackerAction, env_config: PyCREnvConfig, masscan: bool = False) \
            -> Tuple[EnvState, float, bool]:
        """
        ...
        for i in range(env_config.num_retries):
            try:
                ...
                # emulationUtil.delete_cache_file(file_name=cache_result, env_config=env_config)
                # outdata, errdata, total_time = emulationUtil.execute_ssh_cmd(cmd=a.nmap_cmd(),
                #                                                            conn=env_config.emulation_config.agent_conn)
                # emulationUtil.write_estimated_cost(total_time=total_time, action=a, env_config=env_config)
                # env_config.action_costs.add_cost(action_id=a.id, ip=a.ip, cost=round(total_time, 1))
                # time.sleep(env_config.retry_timeout)
                # try:
                #     xml_data = emulationUtil.parse_nmap_scan(file_name=cache_result, env_config=env_config)
                #     scan_result = emulationUtil.parse_nmap_scan_xml(xml_data)
    ...
    @staticmethod
    def nmap_pivot_scan_action_helper(s: EnvState, a: AttackerAction, env_config: PyCREnvConfig, partial_result:
