...
from pycr_common.dao.action_results.ids_alert import IdsAlert
...
class EmulationUtil:
    ...
    def write_user_command_cache(action: AttackerAction, env_config: PyCREnvConfig, user: str, result: str,
        ...
        """
        if jumphost is None:
            jumphost = str(env_config.emulation_config.agent_ip)
    ...
    def delete_cache_file(file_name: str, env_config: PyCREnvConfig) -> Tuple[bytes, bytes, float]:
        ...
        return EmulationUtil.execute_ssh_cmd(cmd=cmd, conn=env_config.emulation_config.agent_conn)
