...
class PyCrCTFLevel5Base:
    """
    Base configuration of level 5 of the PyCrCTF environment. (Mainly used when running in simulation mode
    ...
    def network_conf(generate: bool = False) -> NetworkConfig:
        ...
        agent_reachable = set()
        if not generate:
            nodes = PyCrCTFLevel5Base.nodes()
            adj_matrix = PyCrCTFLevel5Base.adj_matrix()
            agent_reachable = PyCrCTFLevel5Base.agent_reachable()
        network_conf = NetworkConfig(subnet_mask=PyCrCTFLevel5Base.subnet_mask(),
                                     nodes=nodes,
                                     adj_matrix=adj_matrix,
                                     flags_lookup = PyCrCTFLevel5Base.flags_lookup(),
    ...
    def env_config(network_conf : NetworkConfig, attacker_action_conf: AttackerActionConfig,
        ...
        """
        env_config = PyCREnvConfig(network_conf=network_conf, attacker_action_conf=attacker_action_conf, attacker_num_ports_obs=10, attacker_num_vuln_obs=10,
                                   attacker_num_sh_obs=3, num_nodes = PyCrCTFLevel5Base.num_nodes(), render_config=render_conf,
                                   env_mode=EnvMode.SIMULATION,
                                   emulation_config=emulation_config,
                                   simulate_detection=True, detection_reward=10, base_detection_p=0.05,
                                   hacker_ip=PyCrCTFLevel5Base.hacker_ip(), state_type=StateType.BASE,
                                   router_ip=PyCrCTFLevel5Base.router_ip())
