...
class CSLECTFLevel3Base:
    """
    Base configuration of level 3 of the CSLECTF environment. (Mainly used when running in simulation mode
    ...
    def subnet_mask() -> str:
        ...
        """
        subnet_mask = f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}3{constants.CSLE.CSLE_SUBNETMASK_SUFFIX}"
        return subnet_mask
    ...
    def network_conf(generate: bool = False) -> NetworkConfig:
        ...
        agent_reachable = set()
        if not generate:
            nodes = CSLECTFLevel3Base.nodes()
            adj_matrix = CSLECTFLevel3Base.adj_matrix()
            agent_reachable = CSLECTFLevel3Base.agent_reachable()
        network_conf = NetworkConfig(subnet_masks=CSLECTFLevel3Base.subnet_mask(),
                                     nodes=nodes,
    ...
    def env_config(network_conf : NetworkConfig, attacker_action_conf: AttackerActionConfig,
        ...
        env_config = CSLEEnvConfig(network_conf=network_conf, attacker_action_conf=attacker_action_conf,
                                   ...
                                   hacker_ip=CSLECTFLevel3Base.hacker_ip(), state_type=StateType.BASE,
