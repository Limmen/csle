...
class CSLECTFLevel6Base:
    """
    Base configuration of level 6 of the CSLECTF environment. (Mainly used when running in simulation mode
    ...
    def subnet_mask() -> str:
        ...
        """
        subnet_mask = f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6{constants.CSLE.CSLE_SUBNETMASK_SUFFIX}"
        return subnet_mask
    ...
    def hacker_ip() -> str:
        ...
        """
        hacker_ip = f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.191"
        return hacker_ip
    ...
    def router_ip() -> str:
        ...
        """
        router_ip = f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.10"
        return router_ip
    ...
    def flags_lookup() -> str:
        ...
        """
        flags_lookup = {}
        flags_lookup[(f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.2", f"/{constants.COMMANDS.TMP_DIR}/flag2")] = Flag(name="flag2", path=f"/{constants.COMMANDS.TMP_DIR}", id=2, requires_root=False, score=1)
        flags_lookup[(f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.3", f"/{constants.COMMANDS.ROOT_DIR}/flag1")] = Flag(name="flag1", path=f"/{constants.COMMANDS.ROOT_DIR}", id=1, requires_root=True,
                                                           score=1)
        flags_lookup[(f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.79", f"/{constants.COMMANDS.TMP_DIR}/flag3")] = Flag(name="flag3", path=f"/{constants.COMMANDS.TMP_DIR}", id=3, requires_root=False,
                                                           score=1)
        flags_lookup[(f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.54", f"/{constants.COMMANDS.TMP_DIR}/flag4")] = Flag(name="flag4", path=f"/{constants.COMMANDS.TMP_DIR}", id=4, requires_root=False,
                                                           score=1)
        flags_lookup[(f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.61", f"/{constants.COMMANDS.ROOT_DIR}/flag5")] = Flag(name="flag5", path=f"/{constants.COMMANDS.ROOT_DIR}", id=5, requires_root=True,
                                                           score=1)
        flags_lookup[(f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.7", f"/{constants.COMMANDS.TMP_DIR}/flag6")] = Flag(name="flag6", path=f"/{constants.COMMANDS.TMP_DIR}", id=6, requires_root=False,
                                                            score=1)
        return flags_lookup
    ...
    def network_conf(generate: bool = False) -> NetworkConfig:
        ...
        agent_reachable = set()
        if not generate:
            nodes = CSLECTFLevel6Base.nodes()
            adj_matrix = CSLECTFLevel6Base.adj_matrix()
            agent_reachable = CSLECTFLevel6Base.agent_reachable()
        network_conf = NetworkConfig(subnet_masks=CSLECTFLevel6Base.subnet_mask(),
                                     nodes=nodes,
                                     adj_matrix=adj_matrix,
                                     flags_lookup = CSLECTFLevel6Base.flags_lookup(),
                                     agent_reachable=agent_reachable,
                                     vulnerable_nodes=set([f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.3", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.79", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.2",
                                                           f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.54", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.74", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.61",
                                                           f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.62", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.7"])
                                     )
        return network_conf

    @staticmethod
    def agent_reachable() -> set():
        reachable = set([f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.10", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.2", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.3", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.21", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.79"])
        return reachable
    ...
    def emulation_config() -> EmulationConfig:
        ...
        """
        emulation_config = EmulationConfig(server_ip="172.31.212.91", agent_ip=f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.191",
    ...
    def env_config(network_conf : NetworkConfig, attacker_action_conf: AttackerActionConfig,
        ...
        env_config = CSLEEnvConfig(network_conf=network_conf, attacker_action_conf=attacker_action_conf,
                                   ...
                                   attacker_num_sh_obs=3, num_nodes = CSLECTFLevel6Base.num_nodes(), render_config=render_conf,
                                   env_mode=EnvMode.SIMULATION,
                                   emulation_config=emulation_config,
                                   simulate_detection=True, detection_reward=10, base_detection_p=0.05,
                                   hacker_ip=CSLECTFLevel6Base.hacker_ip(), state_type=StateType.BASE,
                                   router_ip=CSLECTFLevel6Base.router_ip())
        ...
        env_config.blacklist_ips = [f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.1", f"{constants.CSLE.CSLE_INTERNAL_SUBNETMASK_PREFIX}6.254"]
