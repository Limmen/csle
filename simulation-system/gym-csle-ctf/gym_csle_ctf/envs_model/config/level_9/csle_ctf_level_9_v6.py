...
class CSLECTFLevel9V6:
    """
    V6 configuration of level 9 of the CSLECTF environment.
    """
    ...
    def attacker_actions_conf(num_nodes : int, subnet_mask: str, hacker_ip: str = None) -> AttackerActionConfig:
        ...
        attacker_actions.append(AttackerNMAPActions.TCP_SYN_STEALTH_SCAN(
            index=num_nodes + 1, ips=subnet_mask, subnet=True))
    ...
    def defender_actions_conf(num_nodes: int, subnet_mask: str) -> DefenderActionConfig:
        ...
        multiple_stop_actions = [DefenderStoppingActions.STOP(index=num_nodes + 1),
                                 DefenderStoppingActions.BLACKLIST_IPS(index=num_nodes + 1),
                                 DefenderStoppingActions.RESET_USERS(index=num_nodes + 1),
                                 DefenderStoppingActions.ENABLE_DPI(index=num_nodes + 1)
                                 ]
        multiple_stop_actions_ids = [DefenderActionId.STOP,
                                 DefenderActionId.BLACKLIST_IPS,
                                 DefenderActionId.RESET_USERS,
                                 DefenderActionId.ENABLE_DPI]
        defender_action_config = DefenderActionConfig(
            ...
            multiple_stop_actions = multiple_stop_actions, multiple_stop_actions_ids=multiple_stop_actions_ids)
    ...
    def env_config(network_conf : NetworkConfig, attacker_action_conf: AttackerActionConfig,
        ...
        env_config.multiple_stopping_environment = True
        env_config.maximum_number_of_defender_stop_actions = 4
        env_config.attacker_prevented_stops_remaining = 1
        return env_config
