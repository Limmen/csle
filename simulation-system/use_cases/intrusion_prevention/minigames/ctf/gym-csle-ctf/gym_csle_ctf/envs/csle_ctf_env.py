...
class CSLECTFEnv(BaseCSLEEnv, metaclass=ABCMeta):
    ...
    def __init__(self, env_config : csleEnvConfig, rs = None):
        ...
        self.attacker_initial_illegal_actions = list(filter(lambda action: not CSLECTFEnv.is_attack_action_legal(
        ...
        self.defender_initial_illegal_actions = list(filter(lambda action: not CSLECTFEnv.is_defense_action_legal(
    ...
    def step(self, action_id : Tuple[int, int]) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[int, int], bool, dict]:
        ...
        defender_info = CSLECTFEnv.initialize_info_dict()
    ...
    def step_attacker(self, attacker_action_id) -> Tuple[int, bool, dict]:
        ...
        if not self.is_attack_action_legal(attacker_action_id, env_config=self.env_config, env_state=self.env_state):
            ...
            print("[Warning] illegal attack action:{}, idx:{}, ip:{},\nillegal actions:{},\nlegal_actions:{}".format(
                ...
                list(filter(lambda action: not CSLECTFEnv.is_attack_action_legal(action, env_config=self.env_config,
                                                                                 env_state=self.env_state), actions)),
                list(filter(lambda action: CSLECTFEnv.is_attack_action_legal(
    ...
    def reset(self, soft : bool = False) -> Tuple[np.ndarray, np.ndarray]:
        ...
        self.attacker_initial_illegal_actions = list(filter(lambda attack_action: not CSLECTFEnv.is_attack_action_legal(
    ...
    def system_identification(self) -> None:
        ...
        self.randomization_space = CSLECTFCSLEDomainRandomizer.generate_randomization_space([self.env_config.network_conf])
        self.env_config.emulation_config.connect_agent()
