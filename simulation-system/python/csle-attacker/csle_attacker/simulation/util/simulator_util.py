...
class SimulatorUtil:
    ...
    @staticmethod
    def reachable_nodes(state: EmulationEnvState) -> Set[str]:
        """
        ...
        """
        reachable_nodes = set()
        agent_reachable = state.emulation_env_config.containers_config.get_agent_reachable_ips()
        logged_in_machines = list(filter(lambda x: x.logged_in and x.tools_installed, state.attacker_obs_state.machines))
        for c in state.emulation_env_config.containers_config.containers:
            for ip in c.get_ips():
