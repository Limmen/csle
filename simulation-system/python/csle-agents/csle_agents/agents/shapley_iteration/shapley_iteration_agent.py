...
class ShapleyIterationAgent(BaseAgent):
    ...
    def hparam_names(self) -> List[str]:
        ...
        return [agents_constants.COMMON.EVAL_BATCH_SIZE, agents_constants.COMMON.CONFIDENCE_INTERVAL,
                ...
                agents_constants.SHAPLEY_ITERATION.STATE_SPACE, agents_constants.SHAPLEY_ITERATION.ACTION_SPACE_PLAYER_1,
    ...
    def shapley_iteration(self, exp_result: ExperimentResult, seed: int) -> ExperimentResult:
        ...
        tabular_policy_p1 = TabularPolicy(player_type=self.experiment_config.player_type,
                                       actions=self.simulation_env_config.joint_action_space_config.action_spaces[
                                          ...
                                          lookup_table=list(maximin_strategies),
                                          simulation_name=self.simulation_env_config.name,
                                       avg_R=V[0])
        tabular_policy_p2 = TabularPolicy(player_type=self.experiment_config.player_type,
                                          actions=self.simulation_env_config.joint_action_space_config.action_spaces[
                                              self.experiment_config.player_idx].actions,
                                          agent_type=self.experiment_config.agent_type, value_function=list(V),
                                          lookup_table=list(minimax_strategies),
                                          simulation_name=self.simulation_env_config.name,
                                          avg_R=V[0])
