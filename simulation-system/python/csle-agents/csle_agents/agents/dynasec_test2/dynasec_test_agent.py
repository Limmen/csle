...
def observation_tensor(n, intrusion_mean, no_intrusion_mean):
    ...
    intrusion_dist = []
    no_intrusion_dist = []
    intrusion_rv = norm(loc=intrusion_mean, scale=1200)
    no_intrusion_rv = norm(loc=no_intrusion_mean, scale=300)
    for i in range(n):
        intrusion_dist.append(intrusion_rv.pdf(i))
        no_intrusion_dist.append(no_intrusion_rv.pdf(i))
    print(f"i mean: {intrusion_mean}, n i mean: {no_intrusion_mean}, sum_int: {sum(intrusion_dist)}, sum_n_int: {sum(no_intrusion_dist)}")
...
class ArrivalThread(threading.Thread):
    ...
    def __init__(self, time_step_len_seconds: float, lamb: int = 10, mu: float = 0.1,
        ...
        self.seed = seed
        self.intrusion_alerts = 800
        self.intrusion_alerts_base = 800
        self.no_intrusion_alerts = 0
        self.baseline_intrusion_alerts = None
        self.baseline_no_intrusion_alerts = None
        self.Z = observation_tensor(n=10000, intrusion_mean=self.intrusion_alerts,
                                              no_intrusion_mean=self.no_intrusion_alerts)
        self.baseline_Z = observation_tensor(n=10000, intrusion_mean=self.intrusion_alerts,
                                    no_intrusion_mean=self.no_intrusion_alerts)

    def sinus_modulated_poisson_rate(self, t):
        return 80 + 60*math.sin(2*math.pi*t*(1/100))
    ...
    def run(self) -> None:
        ...
        while not self.stopped:
            ...
            for i in range(len(self.client_threads)):
                no_intrusion_alerts += random.randint(5, 12)
            self.no_intrusion_alerts = no_intrusion_alerts
            self.intrusion_alerts = int(self.intrusion_alerts_base + self.no_intrusion_alerts*(1+(max(random.random(), 0.7)))
            ...
            self.Z = observation_tensor(n=10000, intrusion_mean=self.intrusion_alerts,
                                        no_intrusion_mean=self.no_intrusion_alerts)
            self.baseline_Z = observation_tensor(n=10000, intrusion_mean=self.baseline_intrusion_alerts,
                                                 no_intrusion_mean=self.baseline_no_intrusion_alerts)
            time.sleep(self.time_step_len_seconds)
            k+= 1
            Logger.__call__().get_logger().info(f"[ArrivalThread] i: {k} clients:{len(self.client_threads)}, rate: {rate}, intrusion_alerts_mean: {self.intrusion_alerts}, "
...
class TrainingThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        self.agent = TSPSAAgent(emulation_env_config=self.emulation_env_config,
                                simulation_env_config=self.simulation_env_config.copy(),
                                experiment_config=self.experiment_config)
        self.experiment_execution = self.agent.train()
        MetastoreFacade.save_experiment_execution(self.experiment_execution)
        self.baseline_policy = list(self.experiment_execution.result.policies.values())[0]
        self.policy = self.baseline_policy.copy()
        avg_return_baseline, avg_upper_bound_return_baseline = self.eval_policy(policy=self.baseline_policy, Z=self.arrival_thread.baseline_Z,
        ...
        self.exp_result.all_metrics[self.seed][agents_constants.COMMON.BASELINE_PREFIX +
                                               agents_constants.COMMON.AVERAGE_RETURN].append(-5)
        self.exp_result.all_metrics[self.seed][agents_constants.COMMON.BASELINE_PREFIX +
                                     agents_constants.COMMON.AVERAGE_RETURN].append(avg_return_baseline)
        self.exp_result.all_metrics[self.seed][agents_constants.COMMON.BASELINE_PREFIX +
                                               agents_constants.COMMON.RUNNING_AVERAGE_RETURN].append(-5)
        self.exp_result.all_metrics[self.seed][agents_constants.COMMON.BASELINE_PREFIX +
                                               agents_constants.COMMON.RUNNING_AVERAGE_RETURN].append(avg_return_baseline)
        self.exp_result.all_metrics[self.seed][agents_constants.COMMON.AVERAGE_RETURN].append(-5)
        self.exp_result.all_metrics[self.seed][agents_constants.COMMON.AVERAGE_RETURN].append(avg_return_baseline)
        self.exp_result.all_metrics[self.seed][agents_constants.COMMON.RUNNING_AVERAGE_RETURN].append(-5)
        self.exp_result.all_metrics[self.seed][agents_constants.COMMON.RUNNING_AVERAGE_RETURN].append(avg_return_baseline)
        self.exp_result.all_metrics[self.seed][env_constants.ENV_METRICS.AVERAGE_UPPER_BOUND_RETURN].append(
            avg_upper_bound_return_baseline)
        self.exp_result.all_metrics[self.seed][env_constants.ENV_METRICS.AVERAGE_UPPER_BOUND_RETURN].append(
            avg_upper_bound_return_baseline)
        i =1
        ...
        while not self.stopped:
            time.sleep(self.sleep_time)
            self.simulation_env_config.simulation_env_input_config.stopping_game_config.Z = self.arrival_thread.Z.copy()
            self.agent = TSPSAAgent(emulation_env_config=self.emulation_env_config,
                                  simulation_env_config=self.simulation_env_config.copy(),
                                  experiment_config=self.experiment_config)
            print("start second train ")
            self.experiment_execution = self.agent.train()
            print("second train complete")
            MetastoreFacade.save_experiment_execution(self.experiment_execution)
            self.policy = list(self.experiment_execution.result.policies.values())[0]
            avg_return_baseline, avg_upper_bound_return_baseline = self.eval_policy(policy=self.baseline_policy, Z=self.arrival_thread.Z,
                                                   sample_Z=self.arrival_thread.baseline_Z, env=self.env, batch=100)
            avg_return, avg_upper_bound_return = self.eval_policy(policy=self.policy, Z=self.arrival_thread.Z,
            ...
            Logger.__call__().get_logger().info(
                f"[DynaSec] i: {i}, "                                                
                f"J:{self.exp_result.all_metrics[self.seed][agents_constants.COMMON.AVERAGE_RETURN][-1]}, "
                f"J_avg_{self.experiment_config.hparams[agents_constants.COMMON.RUNNING_AVERAGE].value}:"
                f"{self.exp_result.all_metrics[self.seed][agents_constants.COMMON.RUNNING_AVERAGE_RETURN][-1]}, "
    ...
    def eval_policy(self, policy, Z, sample_Z, batch, env):
        print(f"policy eval, true Z mean:{DynaSecTestAgent2.mean(Z[0][0][0])}, "
              f"sample Z mean: {DynaSecTestAgent2.mean(sample_Z[0][0][0])}")
        returns = []
        opt_returns = []
        env.config.stopping_game_config.Z = sample_Z
        for i in range(batch):
            ...
            while not done:
                a = policy.action(o=o)
                o, r, done, info = env.step_test(a1=a, sample_Z=Z)
                R+=r
            returns.append(R)
            opt_returns.append(info[env_constants.ENV_METRICS.AVERAGE_UPPER_BOUND_RETURN])
        return float(np.mean(returns)), float(np.mean(opt_returns))


class DynaSecTestAgent2(BaseAgent):
    """
    DynaSec
    """
    ...
    def __init__(self, simulation_env_config: SimulationEnvConfig,
        ...
        assert experiment_config.agent_type == AgentType.DYNA_SEC_TEST2
        self.env = env
    ...
    def train(self) -> ExperimentExecution:
        ...
        p_t = TrainingThread(sleep_time=sleep_time, exp_result=exp_result, arrival_thread=a_t,
                             experiment_config=spsa_experiment_config,
        ...
        while True:
            ...
            self.exp_execution.result = p_t.exp_result
            ...
            self.training_job.experiment_result = p_t.exp_result
