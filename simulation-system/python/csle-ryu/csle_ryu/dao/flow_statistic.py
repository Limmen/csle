...
class FlowStatistic:
    ...
    def __init__(self, timestamp: float, datapath_id: int, in_port: int, out_port: int, dst_mac_address: int,
    ...
    def from_kafka_record(record: str) -> "FlowStatistic":
        ...
        obj = FlowStatistic(timestamp = float(parts[0]), datapath_id=int(parts[1]), in_port=int(parts[2]),
                            out_port=int(parts[3]), dst_mac_address=int(parts[4]), num_packets=int(parts[5]),
