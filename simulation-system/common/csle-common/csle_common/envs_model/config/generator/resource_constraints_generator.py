...
class ResourceConstraintsGenerator:
    ...
    def apply_resource_constraints(resources_config: ResourcesConfig, emulation_config: EmulationConfig):
        ...
        for node_resource_config in resources_config.node_resources_configurations:
            ...
            print(f"appliying resource constraints on node:{node_resource_config.ip}")
            ...
            cmd = f"sudo tc qdisc del dev {node_resource_config.network_config.interface} root netem"
            ...
            cmd = f"sudo tc qdisc add dev {node_resource_config.network_config.interface} root netem " \
                  f"delay {node_resource_config.network_config.packet_delay_ms:.6f}ms " \
                  f"{node_resource_config.network_config.packet_delay_jitter_ms:.6f}ms " \
                  f"distribution {str(node_resource_config.network_config.packet_delay_distribution)} " \
                  f"loss gemodel {node_resource_config.network_config.loss_gemodel_p:.6f} " \
                  f"{node_resource_config.network_config.loss_gemodel_r:.6f} " \
                  f"{(1-node_resource_config.network_config.loss_gemodel_h):.6f} " \
                  f"{(1-node_resource_config.network_config.loss_gemodel_k):.6f} " \
                  f"duplicate {node_resource_config.network_config.packet_duplicate_percentage:.6f}% " \
                  f"{node_resource_config.network_config.packet_duplicate_correlation_percentage:.6f}% corrupt " \
                  f"{node_resource_config.network_config.packet_corrupt_percentage:.6f}% " \
                  f"reorder {node_resource_config.network_config.packet_reorder_percentage:.6f}% " \
                  f"{node_resource_config.network_config.packet_reorder_correlation_percentage:.6f}% " \
                  f"gap {node_resource_config.network_config.packet_reorder_gap} " \
                  f"rate {node_resource_config.network_config.rate_limit_mbit:.6f}mbit " \
                  f"limit {node_resource_config.network_config.limit_packets_queue:.6f}"
            o,e,_ = EmulationUtil.execute_ssh_cmd(cmd=cmd, conn=emulation_config.agent_conn)
