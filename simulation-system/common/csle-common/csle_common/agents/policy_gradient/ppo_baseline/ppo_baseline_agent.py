"""
An agent for the csle env that uses the PPO Policy Gradient algorithm from OpenAI stable baselines
"""
import time
import math
...
import csle_common.agents.policy_gradient.ppo_baseline.impl.ppo.policies # to register policies
...
class PPOBaselineAgent(TrainAgent):
    """
    An agent for the csle-ctf env that uses the PPO Policy Gradient algorithm from OpenAI stable baselines
    """
    ...
    def train(self) -> BaseExperimentResult:
        ...
        if isinstance(self.env, DummyVecEnv):
            ...
            if train_eval_env_i is not None:
                train_eval_env = CSLECTFMonitor(train_eval_env_i, video_dir + "/" + time_str, force=True,
        ...
        if isinstance(self.eval_env, DummyVecEnv) or isinstance(self.eval_env, SubprocVecEnv):
            ...
            if self.eval_env is not None:
                eval_env = CSLECTFMonitor(self.eval_env, video_dir + "/" + time_str, force=True,
                                                    video_frequency=video_frequency, openai_baseline=True)
