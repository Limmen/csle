"""
A general bot attack agent for csle environments that acts greedily according to a pre-trained policy network
"""
import torch
import traceback
import time
...
class PPOAttackerBotAgent:
    ...
    def __init__(self, pg_config: AgentConfig, env_config: BaseCSLEEnvConfig, model_path: str = None,
                 env: BaseCSLEEnv = None):
        """
    ...
    def action(self, s: BaseEnvState, agent_state = None) -> int:
        ...
        try:
            ...
            # non_legal_actions = list(filter(lambda action: not CSLECTFEnv.is_action_legal(
