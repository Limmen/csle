...
class EmulatedAttacker:
    ...
    def attacker_recon_action(s: EmulationEnvState, a: AttackerAction, emulation_env_agent_config: EmulationEnvAgentConfig) -> EmulationEnvState:
        ...
        if a.id == AttackerActionId.TCP_SYN_STEALTH_SCAN_SUBNET or a.id == AttackerActionId.TCP_SYN_STEALTH_SCAN_HOST \
            ...
            return ReconMiddleware.execute_tcp_syn_stealth_scan(
                s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ReconMiddleware.execute_ping_scan(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ReconMiddleware.execute_udp_port_scan(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ReconMiddleware.execute_tcp_fin_scan(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ReconMiddleware.execute_tcp_null_scan(s=s, a=a,
                                                         emulation_env_config=emulation_env_agent_config)
            ...
            return ReconMiddleware.execute_vulscan(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ReconMiddleware.execute_nmap_vulners(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.NIKTO_WEB_HOST_SCAN:
            return ReconMiddleware.execute_nikto_web_host_scan(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.MASSCAN_HOST_SCAN or a.id == AttackerActionId.MASSCAN_SUBNET_SCAN:
            return ReconMiddleware.execute_masscan_scan(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.FIREWALK_HOST or a.id == AttackerActionId.FIREWALK_SUBNET \
                or a.id == AttackerActionId.FIREWALK_ALL:
            return ReconMiddleware.execute_firewalk_scan(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ReconMiddleware.execute_http_enum(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ReconMiddleware.execute_http_grep(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.FINGER_HOST or a.id == AttackerActionId.FINGER_SUBNET \
                or a.id == AttackerActionId.FINGER_ALL:
            return ReconMiddleware.execute_finger(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        else:
    ...
    def attacker_exploit_action(s: EmulationEnvState, a: AttackerAction, emulation_env_agent_config: EmulationEnvAgentConfig) -> EmulationEnvState:
        ...
        if a.id == AttackerActionId.TELNET_SAME_USER_PASS_DICTIONARY_HOST \
            ...
            return ExploitMiddleware.execute_telnet_same_user_dictionary(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ExploitMiddleware.execute_ssh_same_user_dictionary(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ExploitMiddleware.execute_ftp_same_user_dictionary(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ExploitMiddleware.execute_cassandra_same_user_dictionary(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ExploitMiddleware.execute_irc_same_user_dictionary(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ExploitMiddleware.execute_mongo_same_user_dictionary(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ExploitMiddleware.execute_mysql_same_user_dictionary(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ExploitMiddleware.execute_smtp_same_user_dictionary(s=s, a=a, emulation_env_config=emulation_env_agent_config)
            ...
            return ExploitMiddleware.execute_postgres_same_user_dictionary(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.SAMBACRY_EXPLOIT:
            return ExploitMiddleware.execute_sambacry(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.SHELLSHOCK_EXPLOIT:
            return ExploitMiddleware.execute_shellshock(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.DVWA_SQL_INJECTION:
            return ExploitMiddleware.execute_dvwa_sql_injection(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.CVE_2015_3306_EXPLOIT:
            return ExploitMiddleware.execute_cve_2015_3306_exploit(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.CVE_2015_1427_EXPLOIT:
            return ExploitMiddleware.execute_cve_2015_1427_exploit(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.CVE_2016_10033_EXPLOIT:
            return ExploitMiddleware.execute_cve_2016_10033_exploit(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.CVE_2010_0426_PRIV_ESC:
            return ExploitMiddleware.execute_cve_2010_0426_exploit(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.CVE_2015_5602_PRIV_ESC:
            return ExploitMiddleware.execute_cve_2015_5602_exploit(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        else:
    ...
    def attacker_post_exploit_action(s: EmulationEnvState, a: AttackerAction, emulation_env_agent_config: EmulationEnvAgentConfig) -> EmulationEnvState:
        ...
        if a.id == AttackerActionId.NETWORK_SERVICE_LOGIN:
            return PostExploitMiddleware.execute_service_login(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        if a.id == AttackerActionId.FIND_FLAG:
            return PostExploitMiddleware.execute_bash_find_flag(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        if a.id == AttackerActionId.INSTALL_TOOLS:
            return PostExploitMiddleware.execute_install_tools(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        if a.id == AttackerActionId.SSH_BACKDOOR:
            return PostExploitMiddleware.execute_ssh_backdoor(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        else:
    ...
    def attacker_stopping_action(s: EmulationEnvState, a: AttackerAction, emulation_env_agent_config: EmulationEnvAgentConfig) -> EmulationEnvState:
        ...
        if a.id == AttackerActionId.STOP:
            return AttackerStoppingMiddleware.stop_intrusion(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        elif a.id == AttackerActionId.CONTINUE:
            return AttackerStoppingMiddleware.continue_intrusion(s=s, a=a, emulation_env_config=emulation_env_agent_config)
        else:
