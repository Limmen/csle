...
class EmulatedAttacker:
    ...
    def attacker_transition(s: EnvState, attacker_action: AttackerAction, env_config: CSLEEnvConfig) \
        ...
        if attacker_action.type == AttackerActionType.RECON:
            ...
            return EmulatedAttacker.attacker_recon_action(s=s, a=attacker_action, env_config=env_config)
            ...
            if attacker_action.subnet:
                EnvDynamicsUtil.cache_action(env_config=env_config, a=attacker_action, s=s)
            return EmulatedAttacker.attacker_exploit_action(s=s, a=attacker_action, env_config=env_config)
        elif attacker_action.type == AttackerActionType.POST_EXPLOIT:
            return EmulatedAttacker.attacker_post_exploit_action(s=s, a=attacker_action, env_config=env_config)
        elif attacker_action.type == AttackerActionType.STOP or attacker_action.type == AttackerActionType.CONTINUE:
            return EmulatedAttacker.attacker_stopping_action(s=s, a=attacker_action, env_config=env_config)
        else:
