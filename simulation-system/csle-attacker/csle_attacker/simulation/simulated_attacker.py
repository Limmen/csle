...
class SimulatedAttacker:
    ...
    def attacker_transition(s: EmulationEnvState, attacker_action: EmulationAttackerAction,
        ...
        if attacker_action.type == EmulationAttackerActionType.RECON:
            EnvDynamicsUtil.cache_attacker_action(emulation_env_config=emulation_env_config, a=attacker_action, s=s)
            return SimulatedAttacker.attacker_recon_action(s=s, a=attacker_action, emulation_env_config=emulation_env_config)
            ...
            if attacker_action.subnet:
                EnvDynamicsUtil.cache_attacker_action(emulation_env_config=emulation_env_config, a=attacker_action, s=s)
            return SimulatedAttacker.attacker_exploit_action(s=s, a=attacker_action, emulation_env_config=emulation_env_config)
        ...
        elif attacker_action.type == EmulationAttackerActionType.STOP or attacker_action.type == EmulationAttackerActionType.CONTINUE:
            return SimulatedAttacker.attacker_stopping_action(s=s, a=attacker_action, emulation_env_config=emulation_env_config)
        else:
    ...
    def attacker_recon_action(s: EmulationEnvState, a: EmulationAttackerAction,
        ...
        :return: s_prime
        """
        if a.id == EmulationAttackerActionId.TCP_SYN_STEALTH_SCAN_SUBNET \
                or a.id == EmulationAttackerActionId.TCP_SYN_STEALTH_SCAN_HOST \
                or a.id == EmulationAttackerActionId.TCP_SYN_STEALTH_SCAN_ALL:
            return ReconSimulator.simulate_tcp_syn_stealth_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_ping_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_udp_port_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_con_non_stealth_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_fin_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_tcp_null_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_tcp_xmas_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_os_detection_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_vulscan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_nmap_vulners(s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.NIKTO_WEB_HOST_SCAN:
            return ReconSimulator.simulate_nikto_web_host_scan(s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.MASSCAN_HOST_SCAN or a.id == EmulationAttackerActionId.MASSCAN_SUBNET_SCAN:
            return ReconSimulator.simulate_masscan_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_firewalk_scan(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_http_enum(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_http_grep(s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ReconSimulator.simulate_finger(s=s, a=a, emulation_env_config=emulation_env_config)
        else:
            raise ValueError("Recon action id:{},name:{} not recognized".format(a.id, a.name))

    @staticmethod
    def attacker_exploit_action(s: EmulationEnvState, a: EmulationAttackerAction,
                                emulation_env_config: EmulationEnvConfig) -> EmulationEnvState:
        """
        ...
        if a.id == EmulationAttackerActionId.TELNET_SAME_USER_PASS_DICTIONARY_HOST \
            ...
            return ExploitSimulator.simulate_telnet_same_user_dictionary(
                s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ExploitSimulator.simulate_ssh_same_user_dictionary(
                s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ExploitSimulator.simulate_ftp_same_user_dictionary(
                s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ExploitSimulator.simulate_cassandra_same_user_dictionary(
                s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ExploitSimulator.simulate_irc_same_user_dictionary(
                s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ExploitSimulator.simulate_mongo_same_user_dictionary(
                s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ExploitSimulator.simulate_mysql_same_user_dictionary(
                s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ExploitSimulator.simulate_smtp_same_user_dictionary(
                s=s, a=a, emulation_env_config=emulation_env_config)
            ...
            return ExploitSimulator.simulate_postgres_same_user_dictionary(
                s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.SAMBACRY_EXPLOIT:
            return ExploitSimulator.simulate_sambacry_exploit(
                s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.SHELLSHOCK_EXPLOIT:
            return ExploitSimulator.simulate_shellshock_exploit(
                s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.DVWA_SQL_INJECTION:
            return ExploitSimulator.simulate_sql_injection_exploit(
                s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.CVE_2015_3306_EXPLOIT:
            return ExploitSimulator.simulate_cve_2015_3306_exploit(
                s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.CVE_2015_1427_EXPLOIT:
            return ExploitSimulator.simulate_cve_2015_1427_exploit(
                s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.CVE_2016_10033_EXPLOIT:
            return ExploitSimulator.simulate_cve_2016_10033_exploit(
                s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.CVE_2010_0426_PRIV_ESC:
            return ExploitSimulator.simulate_cve_2010_0426_exploit(
                s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.CVE_2015_5602_PRIV_ESC:
            return ExploitSimulator.simulate_cve_2015_5602_exploit(
                s=s, a=a, emulation_env_config=emulation_env_config)
        else:
            raise ValueError("Exploit action id:{},name:{} not recognized".format(a.id, a.name))

    @staticmethod
    def attacker_post_exploit_action(s: EmulationEnvState, a: EmulationAttackerAction,
                                     emulation_env_config: EmulationEnvConfig) -> EmulationEnvState:
        """
        ...
        if a.id == EmulationAttackerActionId.NETWORK_SERVICE_LOGIN:
            s_1, r_1, _ = PostExploitSimulator.simulate_ssh_login(s=s, a=a, emulation_env_config=emulation_env_config)
            s_2, r_2, _ = PostExploitSimulator.simulate_ftp_login(s=s_1, a=a, emulation_env_config=emulation_env_config)
            s_3, r_3, done = PostExploitSimulator.simulate_telnet_login(s=s_2, a=a, emulation_env_config=emulation_env_config)
            return s_3
        if a.id == EmulationAttackerActionId.FIND_FLAG:
            return PostExploitSimulator.simulate_bash_find_flag(s=s, a=a, emulation_env_config=emulation_env_config)
        if a.id == EmulationAttackerActionId.INSTALL_TOOLS:
            return PostExploitSimulator.execute_install_tools(s=s, a=a, emulation_env_config=emulation_env_config)
        if a.id == EmulationAttackerActionId.SSH_BACKDOOR:
            return PostExploitSimulator.execute_ssh_backdoor(s=s, a=a, emulation_env_config=emulation_env_config)
        else:
            raise ValueError("Post-expoit action id:{},name:{} not recognized".format(a.id, a.name))


    @staticmethod
    def attacker_stopping_action(s: EmulationEnvState, a: EmulationAttackerAction,
                                 emulation_env_config: EmulationEnvConfig) -> EmulationEnvState:
        """
        ...
        if a.id == EmulationAttackerActionId.STOP:
            return AttackerStoppingSimulator.stop_intrusion(s=s, a=a, emulation_env_config=emulation_env_config)
        elif a.id == EmulationAttackerActionId.CONTINUE:
            return AttackerStoppingSimulator.continue_intrusion(s=s, a=a, emulation_env_config=emulation_env_config)
        else:
