...
class ShellSimulatorUtil:
    ...
    def simulate_service_login_helper(
            s: EmulationEnvState, a: EmulationAttackerAction, service_name : str = constants.SSH.SERVICE_NAME) \
            -> EmulationEnvState:
        """
            ...
            if access:
                ...
                if new_m_obs.logged_in:
                    for cr in credentials:
                        cr_user = cr.username
                        if cr_user in s.emulation_env_config.users_config.get_root_usernames(ips=c.get_ips()) \
