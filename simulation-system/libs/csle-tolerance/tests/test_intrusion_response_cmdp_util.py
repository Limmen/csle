...
class TestInstrusionToleranceCmdpSuite:
    ...
    def test_cost_tensor(self) -> None:
        ...
        assert IntrusionResponseCmdpUtil.cost_tensor(states,False) == [-item for item in expected]
    ...
    def test_constraint_cost_function(self) -> None:
        ...
        f = 0.5
        expected = [0.0,1.0]
        assert IntrusionResponseCmdpUtil.constraint_cost_function(states[0],f) == expected[0]
        assert IntrusionResponseCmdpUtil.constraint_cost_function(states[1],f) == expected[1]
    ...
    def test_delta_function(self) -> None:
        ...
        delta = 1
        expected = 1.35
        assert round(IntrusionResponseCmdpUtil.delta_function(s,p_a,p_c,p_u,delta,s_max),2) == expected
    ...
    def test_transition_function(self) -> None:
        ...
        p_u = 0.5
        delta = s_prime - a - s
        assert IntrusionResponseCmdpUtil.delta_function(s,p_a,p_c,p_u,delta,s_max) == 3
    ...
    def test_transition_tensor(self) -> None:
        ...
        transition_tensor = IntrusionResponseCmdpUtil.transition_tensor(states,actions,p_a,p_c,p_u,s_max)
