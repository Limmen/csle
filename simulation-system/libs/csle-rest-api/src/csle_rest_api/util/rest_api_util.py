...
def check_if_user_is_authorized(request, requires_admin: bool = False):
    ...
    user = None
    if requires_admin:
        user = MetastoreFacade.get_management_user_by_username(
            username=token_obj.username
        )
    if (
        token_obj is None
        or token_obj.expired(
            valid_length_hours=api_constants.SESSION_TOKENS.EXPIRE_TIME_HOURS
        )
...
def check_if_user_edit_is_authorized(request, user: ManagementUser):
    ...
    token = request.args.get(api_constants.MGMT_WEBAPP.TOKEN_QUERY_PARAM)
    token_obj = MetastoreFacade.get_session_token_metadata(token=token)
    request_user = MetastoreFacade.get_management_user_by_username(
        username=token_obj.username
    )
    if (
        token_obj is None
        ...
        or (not request_user.admin and request_user.username != user.username)
    ):
        if token_obj is not None:
            MetastoreFacade.remove_session_token(session_token=token_obj)
        response = jsonify({})
