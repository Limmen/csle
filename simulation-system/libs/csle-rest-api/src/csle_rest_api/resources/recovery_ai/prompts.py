class Prompts:
    ...
    INCIDENT_PROMPT_TEMPLATE = (
        ...
        f"e.g., security alerts.\n"
        f"Your task is to determine if the logs indicate a cyber incident (i.e., attack) that requires recovery actions.\n"
        f"If the logs are just indicative of normal system activity or if they are unrelated to security, then you "
        f"should classify the logs/system as not being an incident that requires recovery.\n"
        f"Similarly, if the logs contain very minor security alerts that do not warrant any recovery action, "
        f"then you should classify the logs/system as not being an incident that requires recovery.\n"
        f"If there is an incident that requires action, you should concisely describe the incident and explain "
        f"why it is an incident, i.e., you should indicate which parts of the logs or system description indicate "
        f"an incident that requires immediate action.\n"
        ...
        f"You should also associate the incident with tactics and techniques from the MITRE ATT&CK taxonomy. "
        f"You should also identify entities involved in the incident.\n"
    ...
    ACTION_PROMPT_TEMPLATE = (
        f"Below is a system description, a sequence of network logs (e.g., from an intrusion detection system), "
        f"a description of a cybersecurity incident, the current state of the recovery from the incident, "
        f"a list of previously executed recovery actions, "
        f"and an instruction that describes a task.\n"
        f"Write a response that appropriately completes the request.\nBefore generating the response, "
        f"think carefully about the system, the logs, and the instruction, then create a step-by-step "
        f"chain of thoughts to ensure a logical and accurate response.\n\n"
        ...
        f"is_attack_contained: Has the immediate threat been stopped from spreading?\n"
        ...
        "### Instruction:\n"
        "You are a security operator with advanced knowledge in cybersecurity "
        f"and IT systems. You have been given information about a security incident and should"
        f" generate the next suitable action for recovering the system from the incident. "
        f"Your suggested action should be based on the logs, the system description only, the current state, and the "
        f"previous recovery actions.\n"
    ...
    STATE_PROMPT_TEMPLATE = (
        f"Below is a system description, a sequence of network logs (e.g., from an intrusion detection system), "
        f"a description of a cybersecurity incident, the current state of the recovery from the incident, "
        f"a proposed recovery action, and an instruction that describes a task.\n"
        f"Write a response that appropriately completes the request.\nBefore generating the response, "
        f"think carefully about the system, the logs, and the instruction, then create a step-by-step "
        f"chain of thoughts to ensure a logical and accurate response.\n\n"
        "### System:\n{}\n\n"
        ...
        f"the incident, "
        f"and a recovery action.\nYour task is to predict what the next state of the recovery will be after applying "
        f"the recovery action.\n"
        f"For example, if the given recovery action effectively contains the attack and 'is_attack_contained' is "
        f"'false' in the "
        f"current state, then the next state should have 'is_attack_contained' set to 'true'.\nSimilarly, if "
        f"'is_recovered' is 'false' "
        f"in the current state and the given recovery action effectively recovers operational services of the system,"
        f" then the next state "
        f"should have 'is_recovered' set to 'true', etc.\nIt is also possible that multiple state properties change "
        f"values from false to true. "
        f"It is also possible that the state remains the same, i.e., no property changes.\nIt is important that the "
        f"state only changes if the "
        f"action is effective in achieving one of the recovery goals: containment, information gathering, preserving "
        f"evidence, eradication, "
        f"hardening, or recovery.\nA state variable can only change from 'false' to 'true', it cannot be changed from "
        f"'true' to 'false'.\n"
    ...
    GEMINI_ACTION_EVAL = ("Below is a system description, logs, an incident description, a recovery state, and a "
                          ...
                          "### Recovery action:\n{}\n\n"
                          "### Instruction:\n"
                          "Give a score between 0 and 1 that quantifies how good the recovery action is. These are the goals of "
                          "the recovery action:\n"
                          "- It should be concrete and actionable, i.e., it should clearly explain what the security "
                          "operator should do, ideally with details like specific IP addresses/hostnames/configurations/"
                          "vulnerabilities.\n"
