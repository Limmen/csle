...
emulation_executions_bp = Blueprint(
    api_constants.MGMT_WEBAPP.EMULATION_EXECUTIONS_RESOURCE,
    __name__,
...
def emulation_execution(execution_id: int):
    ...
    if emulation is not None:
        execution = MetastoreFacade.get_emulation_execution(
            ip_first_octet=execution_id, emulation_name=emulation
        )
...
@emulation_executions_bp.route(
    f"{constants.COMMANDS.SLASH_DELIM}<execution_id>{constants.COMMANDS.SLASH_DELIM}"
...
def emulation_execution_info(execution_id: int):
    ...
    if emulation is not None:
        ...
        execution_info = ClusterController.get_merged_execution_info(
            execution=execution
        )
        for kibana_tunnel_dto in kibana_tunnels_dto.tunnels:
            if (
                kibana_tunnel_dto.ip
                == execution.emulation_env_config.elk_config.container.docker_gw_bridge_ip
            ):
                execution_info.elk_managers_info.local_kibana_port = (
                    kibana_tunnel_dto.port
                )

        if (
            execution_info.ryu_managers_info is not None
            ...
            for ryu_tunnel_dto in ryu_tunnels_dto.tunnels:
                if (
                    ...
                    execution_info.ryu_managers_info.local_controller_web_port = (
                        ryu_tunnel_dto.port
                    )
...
def start_stop_client_manager(execution_id: int):
    ...
    # Verify payload
    if (
        api_constants.MGMT_WEBAPP.IP_PROPERTY not in json_data
        or api_constants.MGMT_WEBAPP.START_PROPERTY not in json_data
        or api_constants.MGMT_WEBAPP.STOP_PROPERTY not in json_data
    ):
...
def start_stop_docker_stats_manager(execution_id: int):
    ...
    # Verify payload
    if (
        api_constants.MGMT_WEBAPP.START_PROPERTY not in json_data
        or api_constants.MGMT_WEBAPP.STOP_PROPERTY not in json_data
    ):
        return jsonify({}), constants.HTTPS.BAD_REQUEST_STATUS_CODE
    config = MetastoreFacade.get_config(id=1)
...
def start_stop_container(execution_id: int):
    ...
    if (
        api_constants.MGMT_WEBAPP.NAME_PROPERTY not in json_data
        or api_constants.MGMT_WEBAPP.START_PROPERTY not in json_data
        or api_constants.MGMT_WEBAPP.STOP_PROPERTY not in json_data
    ):
        return jsonify({}), constants.HTTPS.BAD_REQUEST_STATUS_CODE
...
def start_stop_elk_manager(execution_id: int):
    ...
    if emulation is not None:
        ...
        if (
            len(execution_info.elk_managers_info.elk_managers_statuses) > 0
            and execution_info.elk_managers_info.elk_managers_statuses[0].kibanaRunning
        ):
            ClusterController.create_kibana_tunnel(
                ip=execution.emulation_env_config.elk_config.container.physical_host_ip,
                port=constants.GRPC_SERVERS.CLUSTER_MANAGER_PORT,
...
def start_stop_ryu_manager(execution_id: int):
    ...
    if emulation is not None:
        ...
        if (
            len(execution_info.ryu_managers_info.ryu_managers_statuses) > 0
            and execution_info.ryu_managers_info.ryu_managers_statuses[0].ryu_running
        ):
            ClusterController.create_ryu_tunnel(
                ip=execution.emulation_env_config.sdn_controller_config.container.physical_host_ip,
                port=constants.GRPC_SERVERS.CLUSTER_MANAGER_PORT,
...
def start_stop_ryu_controller(execution_id: int):
    ...
    if emulation is not None:
        ...
        if (
            ...
            execution_info.ryu_managers_info.ryu_managers_statuses[
                0
    ...
    if (
        ...
        if int(execution.ip_first_octet) == int(execution_id):
            ...
            for dpid in switches:
                ...
                sw_dict[api_constants.MGMT_WEBAPP.DESC_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.FLOWS_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.AGG_FLOWS_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)][0]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                tables = list(
                    filter(
                        lambda x: x[api_constants.MGMT_WEBAPP.ACTIVE_COUNT_PROPERTY]
                        > 0,
                        tables,
                    )
                )
                filtered_table_ids = list(
                    map(
                        lambda x: x[api_constants.MGMT_WEBAPP.TABLE_ID_PROPERTY], tables
                    )
                )
                ...
                tablefeatures = list(
                    filter(
                        lambda x: x[api_constants.MGMT_WEBAPP.TABLE_ID_PROPERTY]
                        in filtered_table_ids,
                        tablefeatures,
                    )
                )
                sw_dict[
                    api_constants.MGMT_WEBAPP.TABLE_FEATURES_PROPERTY
                ] = tablefeatures
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                    f"{execution.emulation_env_config.sdn_controller_config.container.physical_host_ip}:"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.PORT_STATS_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.PORT_DESCS_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.QUEUES_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.QUEUE_CONFIGS_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)][0]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.GROUPS_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.GROUP_DESCS_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.GROUP_FEATURES_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)][0]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.METERS_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.METER_CONFIGS_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.METER_FEATURES_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)][0]
                response = requests.get(
                    f"{constants.HTTP.HTTP_PROTOCOL_PREFIX}"
                ...
                sw_dict[api_constants.MGMT_WEBAPP.ROLES_PROPERTY] = json.loads(
                    response.content
                )[str(dpid)][0]
                switches_dicts.append(sw_dict)
            response_data = {}
            response_data[
                api_constants.MGMT_WEBAPP.SWITCHES_SUBRESOURCE
            ] = switches_dicts
            response_data[
                api_constants.MGMT_WEBAPP.SDN_CONTROLLER_LOCAL_PORT
            ] = local_ryu_port
