...
from csle_rest_api.pages.emulation_statistics.routes import (
    get_emulation_statistics_page_bp,
)
...
from csle_rest_api.resources.gaussian_mixture_system_models.routes import (
    gaussian_mixture_system_models_bp,
)
from csle_rest_api.resources.empirical_system_models.routes import (
    empirical_system_models_bp,
)
...
from csle_rest_api.resources.multi_threshold_policies.routes import (
    multi_threshold_policies_bp,
)
from csle_rest_api.resources.linear_threshold_policies.routes import (
    linear_threshold_policies_bp,
)
...
from csle_rest_api.resources.fnn_w_softmax_policies.routes import (
    fnn_w_softmax_policies_bp,
)
...
from csle_rest_api.resources.system_identification_jobs.routes import (
    system_identification_jobs_bp,
)
from csle_rest_api.resources.emulation_simulation_traces.routes import (
    emulation_simulation_traces_bp,
)
...
from csle_rest_api.web_sockets.container_terminal.container_terminal import (
    get_container_terminal_bp,
)
...
def create_app(static_folder: str):
    ...
    app.register_blueprint(
        get_emulations_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.EMULATIONS_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_simulations_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.SIMULATIONS_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_traces_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.TRACES_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_monitoring_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.MONITORING_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_emulation_statistics_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.EMULATION_STATISTICS_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_system_models_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.SYSTEM_MODELS_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_about_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.ABOUT_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_login_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.LOGIN_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_downloads_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.DOWNLOADS_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_server_cluster_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
    ...
    app.register_blueprint(
        get_images_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.IMAGES_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_jobs_page_bp(static_folder=f"../../{static_folder}"),
    ...
    app.register_blueprint(
        get_policies_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.POLICIES_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_policy_examination_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
    ...
    app.register_blueprint(
        get_training_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.TRAINING_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_sdn_controllers_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.SDN_CONTROLLERS_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_control_plane_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.CONTROL_PLANE_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_user_admin_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.USER_ADMIN_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_container_terminal_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.CONTAINER_TERMINAL_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_system_admin_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.SYSTEM_ADMIN_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_logs_admin_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
        f"{api_constants.MGMT_WEBAPP.LOGS_ADMIN_PAGE_RESOURCE}",
    )
    app.register_blueprint(
        get_register_page_bp(static_folder=f"../../{static_folder}"),
        url_prefix=f"{constants.COMMANDS.SLASH_DELIM}"
...
def start_server(
    ...
    Logger.__call__().get_logger().info(
        f"Starting web server, listening on port: {port}"
    )
