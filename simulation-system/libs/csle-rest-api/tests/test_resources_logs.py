...
class TestResourcesLogsSuite:
    ...
    @pytest.fixture
    def container_logs(self, mocker):
        """
        Pytest fixture for mocking the get_ryu_controller_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_ryu_controller_logs(ip, port, emulation, ip_first_octet,
                                    container_ip,) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.CONTAINER_SUBRESOURCE}":
                    654321}
        get_ryu_controller_logs_mocker = mocker.MagicMock(side_effect=get_ryu_controller_logs)
        return get_ryu_controller_logs_mocker

    @pytest.fixture
    def client_manager_logs(self, mocker):
        """
        Pytest fixture for mocking the get_client_manager function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_client_manager_logs(ip, port, emulation, ip_first_octet,
                                    ) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.CLIENT_MANAGER_SUBRESOURCE}":
                    654321}
        get_client_manager_logs_mocker = mocker.MagicMock(
            side_effect=get_client_manager_logs)
        return get_client_manager_logs_mocker

    @pytest.fixture
    def kafka_manager_logs(self, mocker):
        """
        Pytest fixture for mocking the get_kafka_manager function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_kafka_manager_logs(ip, port, emulation, ip_first_octet,
                                   ) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.KAFKA_MANAGER_SUBRESOURCE}":
                    654321}
        get_kafka_manager_logs_mocker = mocker.MagicMock(
            side_effect=get_kafka_manager_logs)
    ...
    def kafka_logs(self, mocker):
        ...
        :return: the mocked function
        """
        def get_kafka_logs(ip, port, emulation, ip_first_octet,) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.KAFKA_SUBRESOURCE}":
                    654321}
        get_kafka_logs_mocker = mocker.MagicMock(
            side_effect=get_kafka_logs)
        return get_kafka_logs_mocker

    @pytest.fixture
    def snort_ids_manager_logs(self, mocker):
        """
        Pytest fixture for mocking the get_snort_ids_manager_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_snort_ids_manager_logs(ip, port, emulation, ip_first_octet,
                                       container_ip,) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.SNORT_IDS_MANAGER_SUBRESOURCE}":
                    654321}
        get_snort_ids_manager_logs_mocker = mocker.MagicMock(
            side_effect=get_snort_ids_manager_logs)
        return get_snort_ids_manager_logs_mocker

    @pytest.fixture
    def snort_ids_logs(self, mocker):
        """
        Pytest fixture for mocking the get_snort_ids_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_snort_ids_logs(ip, port, emulation, ip_first_octet,
                               container_ip,) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.SNORT_IDS_SUBRESOURCE}":
                    654321}
        get_snort_ids_logs_mocker = mocker.MagicMock(
            side_effect=get_snort_ids_logs)
        return get_snort_ids_logs_mocker

    @pytest.fixture
    def ossec_ids_manager_logs(self, mocker):
        """
        Pytest fixture for mocking the get_ossec_ids_manager_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_ossec_ids_manager_logs(ip, port, emulation, ip_first_octet,
                                       container_ip,) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.OSSEC_IDS_MANAGER_SUBRESOURCE}":
                    654321}
        get_ossec_ids_manager_logs_mocker = mocker.MagicMock(
            side_effect=get_ossec_ids_manager_logs)
        return get_ossec_ids_manager_logs_mocker

    @pytest.fixture
    def ossec_ids_logs(self, mocker):
        """
        Pytest fixture for mocking the get_ossec_ids_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_ossec_ids_logs(ip, port, emulation, ip_first_octet,
                               container_ip,) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.OSSEC_IDS_SUBRESOURCE}":
                    654321}
        get_ossec_ids_logs_mocker = mocker.MagicMock(
            side_effect=get_ossec_ids_logs)
        return get_ossec_ids_logs_mocker

    @pytest.fixture
    def host_manager_logs(self, mocker):
        """
        Pytest fixture for mocking the get_host_manager_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_host_manager_logs(ip, port, emulation, ip_first_octet,
                                  container_ip,) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.HOST_MANAGER_SUBRESOURCE}":
                    654321}
        get_host_manager_logs_mocker = mocker.MagicMock(
            side_effect=get_host_manager_logs)
        return get_host_manager_logs_mocker

    @pytest.fixture
    def traffic_manager_logs(self, mocker):
        """
        Pytest fixture for mocking the get_traffic_manager_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_traffic_manager_logs(ip, port, emulation, ip_first_octet,
                                     container_ip,) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.TRAFFIC_MANAGER_SUBRESOURCE}":
                    654321}
        get_traffic_manager_logs_mocker = mocker.MagicMock(
            side_effect=get_traffic_manager_logs)
        return get_traffic_manager_logs_mocker

    @pytest.fixture
    def elk_manager_logs(self, mocker):
        """
        Pytest fixture for mocking the get_elk_manager_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_elk_manager_logs(ip, port, emulation, ip_first_octet,
                                 ) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.TRAFFIC_MANAGER_SUBRESOURCE}":
                    654321}
        get_elk_manager_logs_mocker = mocker.MagicMock(
            side_effect=get_elk_manager_logs)
        return get_elk_manager_logs_mocker

    @pytest.fixture
    def elk_logs(self, mocker):
        """
        Pytest fixture for mocking the get_elk_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_elk_logs(ip, port, emulation, ip_first_octet,
                         ) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.TRAFFIC_MANAGER_SUBRESOURCE}":
                    654321}
        get_elk_logs_mocker = mocker.MagicMock(
            side_effect=get_elk_logs)
        return get_elk_logs_mocker

    @pytest.fixture
    def ryu_manager_logs(self, mocker):
        """
        Pytest fixture for mocking the get_ryu_manger_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        """
        def get_ryu_manager_logs(ip, port, emulation, ip_first_octet,
                                 ) -> Dict[str, Any]:
            return {f"{api_constants.MGMT_WEBAPP.RYU_MANAGER_SUBRESOURCE}":
                    654321}
        get_ryu_manager_logs_mocker = mocker.MagicMock(
            side_effect=get_ryu_manager_logs)
        return get_ryu_manager_logs_mocker

    @pytest.fixture
    def ryu_controller_logs(self, mocker):
        """
        Pytest fixture for mocking the get_ryu_controller_logs function
        :param mocker: the pytest mocker object
        :return: the mocked function
        ...
        def get_ryu_controller_logs(ip, port, emulation, ip_first_octet,
            ...
            return {f"{api_constants.MGMT_WEBAPP.RYU_CONTROLLER_SUBRESOURCE}":
                    654321}
        get_ryu_controller_logs_mocker = mocker.MagicMock(
            side_effect=get_ryu_controller_logs)
        return get_ryu_controller_logs_mocker
    ...
    def get_em_ex(self, mocker):
        ...
        def get_emulation_execution(ip_first_octet: int,
                                    emulation_name: str) -> EmulationExecution:
            c_net = ContainerNetwork(name="JohnDoe", subnet_mask="JDoeSubnet", bitmask="null",
                                     subnet_prefix="null", interface="eth0")
            nc_config = NodeContainerConfig(name="JohnDoe",
                                            ips_and_networks=[("null", c_net)],
            ...
            n_u_config = NodeUsersConfig(ip="123.456.78.99",
                                         users=[User(username="JDoe", pw="JDoe",
                                                     root=True)],
                                         docker_gw_bridge_ip="null",
                                         physical_host_ip="123.456.78.99")
            u_config = UsersConfig(users_configs=[n_u_config])
            flag = Flag(name="JohnDoe",
                        dir="null",
                        id=1,
                        path="null",
                        requires_root=False,
                        score=1)
            nf_conf = NodeFlagsConfig(ip="123.456.78.99",
                                      flags=[flag],
                                      docker_gw_bridge_ip="null",
                                      physical_host_ip="123.456.78.99")
            cred = Credential(username="JDoe", pw="JDoe",
                              port=None, protocol=None,
                              service="null", root=False)
            nv_conf = NodeVulnerabilityConfig(ip="123.456.78.99",
                                              vuln_type=VulnType(0),
                                              name="JohnDoe",
                                              port=1,
                                              protocol=TransportProtocol(0),
                                              credentials=[cred],
                                              cvss=2.0,
                                              cve=None,
                                              service="null",
                                              ...
                                              docker_gw_bridge_ip="123.456.78.99",
                                              physical_host_ip="123.456.78.99")
            dfn_conf = DefaultNetworkFirewallConfig(ip=None, default_gw="null",
                                                    default_input="null",
                                                    default_output="null",
            ...
            node_traf_conf = NodeTrafficConfig(ip="123.456.78.99",
                                               ...
                                               docker_gw_bridge_ip="123.456.78.99",
                                               physical_host_ip="123.456.78.99")
            nn_config = NodeNetworkConfig(interface:=constants.NETWORKING.ETH0,
                                          limit_packets_queue=30000, packet_delay_ms=0.1,
                                          packet_delay_jitter_ms=0.025,
            ...
            kafka_top = KafkaTopic(name="JohhnDoe", num_partitions=3,
                                   num_replicas=5,
                                   attributes=["null"],
                                   retention_time_hours=3)
            kafka_conf = KafkaConfig(container=nc_config,
                                     resources=node_res_conf,
                                     firewall_config=n_fire_conf,
                                     topics=[kafka_top],
                                     kafka_manager_log_file="null",
                                     kafka_manager_log_dir="null",
                                     kafka_manager_max_workers=9,
                                     kafka_port=9092,
                                     kafka_port_external=9292,
                                     time_step_len_seconds=15,
                                     kafka_manager_port=50051,
                                     version="0.0.1")
            network_service = NetworkService(protocol=TransportProtocol(0),
                                             port=1,
                                             name="JohnDoe",
                                             credentials=None)
            ...
            e_a_action = EmulationAttackerAction(id=EmulationAttackerActionId(0), name="JohnDoe",
                                                 ...
                                                 execution_time=0.0,
                                                 ts=1.1)
            ovs_switch = OvsSwitchConfig(container_name="JohnDoe",
                                         ip="123.456.78.99",
                                         openflow_protocols=["null"],
                                         controller_ip="123.456.78.99",
                                         controller_port=2,
                                         controller_transport_protocol="null",
                                         docker_gw_bridge_ip="null",
                                         physical_host_ip="123.456.78.99")
            sdc_ctrl = SDNControllerConfig(container=nc_config, resources=node_res_conf,
                                           firewall_config=n_fire_conf,
                                           controller_port=4,
                                           controller_type=SDNControllerType(0),
                                           controller_module_name="null",
                                           controller_web_api_port=5,
                                           manager_log_file="null",
                                           manager_log_dir="null",
                                           manager_max_workers=10,
                                           time_step_len_seconds=15,
                                           version="0.0.1",
                                           manager_port=50042)
            host_mng = HostManagerConfig(host_manager_log_file="null",
                                         host_manager_log_dir="null",
                                         host_manager_max_workers=5,
                                         time_step_len_seconds=15,
                                         host_manager_port=50049,
                                         version="0.0.1")
            snort_mng = SnortIDSManagerConfig(snort_ids_manager_log_file="null",
                                              snort_ids_manager_log_dir="null",
                                              snort_ids_manager_max_workers=5,
                                              time_step_len_seconds=15,
                                              snort_ids_manager_port=50048,
                                              version="0.0.1")
            ossec_mng = OSSECIDSManagerConfig(ossec_ids_manager_log_file="null",
                                              ossec_ids_manager_log_dir="null",
                                              ossec_ids_manager_max_workers=5,
                                              time_step_len_seconds=15,
            ...
            docker_mng = DockerStatsManagerConfig(docker_stats_manager_log_file="null",
                                                  ...
                                                  docker_stats_manager_max_workers=5,
                                                  time_step_len_seconds=15,
                                                  docker_stats_manager_port=50046,
                                                  version="0.0.1")
            elk = ElkConfig(container=nc_config,
                            resources=node_res_conf,
                            firewall_config=n_fire_conf,
                            elk_manager_log_file="null",
                            elk_manager_log_dir="null",
                            elk_manager_max_workers=5,
                            elastic_port=9200,
                            kibana_port=5601,
                            logstash_port=5044,
                            time_step_len_seconds=15,
                            ...
                            version="0.0.1")
            nb_config = NodeBeatsConfig(ip="123.456.78.99",
                                        log_files_paths=["null"],
                                        filebeat_modules=["null"],
                                        metricbeat_modules=["null"],
                                        heartbeat_hosts_to_monitor=["null"],
                                        kafka_input=False,
            ...
            beats = BeatsConfig(node_beats_configs=[nb_config],
                                num_elastic_shards=3,
                                reload_enabled=True)
            em_env = EmulationEnvConfig(name="JohnDoe",
                                        containers_config=c_config,
                                        users_config=u_config,
                                        flags_config=FlagsConfig(node_flag_configs=[nf_conf]),
                                        vuln_config=VulnerabilitiesConfig(node_vulnerability_configs=[nv_conf]),
                                        topology_config=top_conf,
                                        traffic_config=traf_conf,
                                        resources_config=res_conf,
                                        kafka_config=kafka_conf,
                                        services_config=service_conf,
                                        descr="null",
                                        static_attacker_sequences={str: [e_a_action]},
                                        ovs_config=OVSConfig(switch_configs=[ovs_switch]),
                                        sdn_controller_config=sdc_ctrl,
                                        host_manager_config=host_mng,
                                        snort_ids_manager_config=snort_mng,
                                        ossec_ids_manager_config=ossec_mng,
                                        docker_stats_manager_config=docker_mng,
                                        elk_config=elk,
                                        beats_config=beats,
            ...
            em_ex = EmulationExecution(emulation_name="JohnDoeEmulation",
                                       timestamp=1.5, ip_first_octet=10,
                                       emulation_env_config=em_env,
                                       physical_servers=["JohnDoeServer"])
            return em_ex
        get_emulation_execution_mocker = mocker.MagicMock(
            side_effect=get_emulation_execution)
    ...
    def test_logs_post_cluster(self, flask_app, not_logged_in, logged_in,
                               logged_in_as_admin,
    ...
    def test_logs_post_metastore(self, flask_app, not_logged_in, logged_in,
                                 logged_in_as_admin,
                                 mocker: pytest_mock.MockFixture,
                                 example_config, csle_logs,
                                 get_em_ex, container_logs, client_manager_logs,
                                 kafka_manager_logs, kafka_logs,
                                 snort_ids_manager_logs,
                                 snort_ids_logs, ossec_ids_manager_logs,
                                 ossec_ids_logs, host_manager_logs,
                                 traffic_manager_logs, elk_manager_logs,
                                 elk_logs, ryu_manager_logs,
                                 ryu_controller_logs,
                                 config) -> None:
        """
        Tests the POST HTTPS method for the /logs resource
        ...
        corr_func_names = ["get_client_manager_logs", "get_kafka_manager_logs", "get_kafka_logs",
                           "get_snort_ids_manager_logs", "get_snort_ids_logs",
                           "get_ossec_ids_manager_logs", "get_ossec_ids_logs",
                           "get_host_manager_logs", "get_traffic_manager_logs",
                           "get_elk_manager_logs", "get_elk_logs", "get_ryu_manager_logs",
        ...
        response = flask_app.test_client().post(f"{api_constants.MGMT_WEBAPP.LOGS_RESOURCE}"
                                                ...
                                                f"?{api_constants.MGMT_WEBAPP.EMULATION_QUERY_PARAM}=JohnDoe"
        ...
        assert response.status_code == constants.HTTPS.BAD_REQUEST_STATUS_CODE
        assert response_data_dict == {}
        config_cluster_dict[api_constants.MGMT_WEBAPP.NAME_PROPERTY] = \
            "JohnDoe"
        response = flask_app.test_client().post(f"{api_constants.MGMT_WEBAPP.LOGS_RESOURCE}"
                                                f"{constants.COMMANDS.SLASH_DELIM}"
                                                f"{api_constants.MGMT_WEBAPP.CONTAINER_SUBRESOURCE}"
        ...
        for i in range(len(mocker_list)):
            ...
            assert response.status_code == constants.HTTPS.BAD_REQUEST_STATUS_CODE
            assert response_data_dict == {}
            indata_dict = {api_constants.MGMT_WEBAPP.NAME_PROPERTY: "JohnDoe"}
            response = flask_app.test_client().post(f"{api_constants.MGMT_WEBAPP.LOGS_RESOURCE}"
                                                    f"{constants.COMMANDS.SLASH_DELIM}"
                                                    f"{constants_list[i]}"
                                                    f"?{api_constants.MGMT_WEBAPP.EMULATION_QUERY_PARAM}=JohnDoe"
            ...
            response_data = response.data.decode("utf-8")
            response_data_dict = json.loads(response_data)
            if corr_func_names[i] == "get_snort_ids_manager_logs":
                assert response.status_code == constants.HTTPS.BAD_REQUEST_STATUS_CODE
                assert response_data_dict == {}
            else:
                assert response.status_code == constants.HTTPS.OK_STATUS_CODE
