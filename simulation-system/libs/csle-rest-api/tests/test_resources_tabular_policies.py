...
class TestRecourcesTabularSuite:
    ...
    @pytest.fixture
    def list_tabular_ids(self, mocker):
        """
    ...
    def remove(self, mocker):
        ...
        """
        def remove_tabular_policy(tabular_policy):
            return None
    ...
    def test_tabular_policies_get(
        ...
        mocker.patch("csle_common.metastore.metastore_facade.MetastoreFacade."
                     "list_tabular_policies",
                     side_effect=list_tabular)
        mocker.patch("csle_common.metastore.metastore_facade.MetastoreFacade." +
                     "list_tabular_policies_ids",
        ...
        assert tab_dict["id"] == 111
        assert tab_dict["simulation"] == "some_simulation"
        response = flask_app.test_client().get(api_constants.MGMT_WEBAPP.TABULAR_POLICIES_RESOURCE)
        response_data = response.data.decode("utf-8")
        response_data_list = json.loads(response_data)
    ...
    def test_tabular_policies_delete(
            self,
            flask_app,
            mocker,
            list_tabular,
    ...
    def test_tabular_policies_id_get(self, flask_app, mocker, logged_in,
        ...
        response = flask_app.test_client().get(f"{api_constants.MGMT_WEBAPP.TABULAR_POLICIES_RESOURCE}"f"/10")
        response_data = response.data.decode("utf-8")
        response_data_list = json.loads(response_data)
        assert response.status_code == constants.HTTPS.UNAUTHORIZED_STATUS_CODE
        assert response_data_list == {}
    ...
    def test_tabular_policies_id_delete(self, flask_app, mocker, logged_in,
                                        not_logged_in, logged_in_as_admin,
        ...
        response = flask_app.test_client().delete(f"{api_constants.MGMT_WEBAPP.TABULAR_POLICIES_RESOURCE}"
                                                  f"/10")
        response_data = response.data.decode("utf-8")
        response_data_list = json.loads(response_data)
        assert response.status_code == constants.HTTPS.UNAUTHORIZED_STATUS_CODE
