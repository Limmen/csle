...
class TestResourcesFileSuite:
    ...
    def test_file_post(self, mocker, flask_app, not_logged_in,
                       logged_in, logged_in_as_admin, true_path, false_path,
                       op):
        """
        ...
        response = flask_app.test_client().post(api_constants.MGMT_WEBAPP.FILE_RESOURCE,
                                                data=json.dumps({api_constants.MGMT_WEBAPP.PATH_PROPERTY:
                                                                 "null"}))
        ...
        response = flask_app.test_client().post(api_constants.MGMT_WEBAPP.FILE_RESOURCE,
                                                data=json.dumps({api_constants.MGMT_WEBAPP.PATH_PROPERTY:
                                                                 "/home/nils/csle/examples/examples.txt"}))
        response_data = response.data.decode("utf-8")
        response_data_dict = json.loads(response_data)
