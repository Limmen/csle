...
class TestResourcesSystemIdentificationSuite:
    ...
    pytest.logger = logging.getLogger("resources_system_identification_jobs_tests")
    ...
    @pytest.fixture
    def get_job_config(self, mocker) -> SystemIdentificationJobConfig:
        """
    ...
    def test_s_i_jobs_get(self, flask_app, mocker, list_jobs,
    ...
    def test_s_i_jobs_delete(self, flask_app, mocker, list_jobs, logged_in,
    ...
    def test_s_i_jobs_id_get(self, flask_app, mocker, logged_in, not_logged_in, logged_in_as_admin,
    ...
    def test_s_i_jobs_id_delete(self, flask_app, mocker, logged_in_as_admin, logged_in, not_logged_in,
        ...
        mocker.patch("csle_common.metastore.metastore_facade.MetastoreFacade."
                     "get_system_identification_job_config",
                     side_effect=get_job_config)
        mocker.patch("csle_cluster.cluster_manager.cluster_controller."
                     "ClusterController.stop_pid",
                     side_effect=stop)
        mocker.patch("csle_common.metastore.metastore_facade.MetastoreFacade."
                     "remove_system_identification_job",
                     side_effect=remove)
        mocker.patch("csle_rest_api.util.rest_api_util.check_if_user_is_authorized",
                     side_effect=not_logged_in)
        response = flask_app.test_client().delete(f"{api_constants.MGMT_WEBAPP.SYSTEM_IDENTIFICATION_JOBS_RESOUCE}"
                                                  f"/10")
        response_data = response.data.decode("utf-8")
        response_data_list = json.loads(response_data)
        assert response.status_code == constants.HTTPS.UNAUTHORIZED_STATUS_CODE
    ...
    def test_s_i_jobs_id_post(self, flask_app, mocker, logged_in_as_admin, logged_in, not_logged_in,
