...
class TestResourcesGrafanaSuite(object):
    ...
    def config(self, mocker):
        ...
        """
        def get_config(id):
            config = TestResourcesGrafanaSuite.example_config()
            return config
    ...
    @pytest.fixture
    def node_status_grafana_running(self, mocker):
        """
        ...
        def get_node_status(ip: str, port: int) -> NodeStatusDTO:
            node_status = TestResourcesGrafanaSuite.example_node_status()
    ...
    @pytest.fixture
    def node_status_grafana_not_running(self, mocker):
        """
    ...
    def test_grafana_get(self, flask_app, mocker, logged_in_as_admin, logged_in, not_logged_in, config,
                         node_status_grafana_running, node_status_grafana_not_running, start, stop) -> None:
        """
        ...
        mocker.patch(
            "csle_cluster.cluster_manager.cluster_controller.ClusterController.start_grafana",
            side_effect=start,
        )
        mocker.patch(
            "csle_cluster.cluster_manager.cluster_controller.ClusterController.stop_grafana",
            side_effect=stop,
        )
    ...
    def test_grafana_post(self, flask_app, mocker, logged_in_as_admin, logged_in, not_logged_in, config,
                          node_status_grafana_running, node_status_grafana_not_running, start, stop) -> None:
        """
