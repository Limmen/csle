...
class TestBayesOptSuite(object):
    ...
    def experiment_config(self) -> ExperimentConfig:
        ...
        experiment_config = ExperimentConfig(
            ...
            log_every=1,
            hparams={
                agents_constants.BAYESIAN_OPTIMIZATION.N: HParam(value=5, name=agents_constants.T_SPSA.N,
    ...
    def test_run_agent(self, mocker, experiment_config: ExperimentConfig,
        ...
        for l in range(1, experiment_config.hparams[agents_constants.BAYESIAN_OPTIMIZATION.L].value + 1):
            assert f"{env_constants.ENV_METRICS.STOP}_{l}" in experiment_execution.result.plot_metrics
            assert (f"{env_constants.ENV_METRICS.STOP}_running_average_{l}" in
                    experiment_execution.result.plot_metrics)
        for seed in experiment_config.random_seeds:
            ...
            for l in range(1, experiment_config.hparams[agents_constants.BAYESIAN_OPTIMIZATION.L].value + 1):
                assert (f"{agents_constants.BAYESIAN_OPTIMIZATION.STOP_DISTRIBUTION_DEFENDER}_l={l}"
                        in experiment_execution.result.all_metrics[seed])
            ...
            assert (env_constants.ENV_METRICS.AVERAGE_DEFENDER_BASELINE_STOP_ON_FIRST_ALERT_RETURN in
                    experiment_execution.result.all_metrics[seed])
            for l in range(1, experiment_config.hparams[agents_constants.BAYESIAN_OPTIMIZATION.L].value + 1):
                assert f"{env_constants.ENV_METRICS.STOP}_{l}" in experiment_execution.result.all_metrics[seed]
                assert (f"{env_constants.ENV_METRICS.STOP}_running_average_{l}" in
                        experiment_execution.result.all_metrics[seed])
