...
class TestSDNControllerSuite:
    ...
    def test_start_ryu_manager(self, mock_execute_ssh_cmd, mock_connect_admin) -> None:
        ...
        sdn_controller_config = MagicMock(
            ...
            manager_port=6633,
            manager_log_dir="/var/log/ryu",
            manager_log_file="ryu.log",
    ...
    def test_stop_ryu_manager(self, mock_execute_ssh_cmd, mock_connect_admin) -> None:
        ...
        mock_execute_ssh_cmd.side_effect = [
            (b"", b"", 0),  # output of pkill ryu_manager
        ]
    ...
    @patch(
        "csle_common.controllers.sdn_controller_manager.SDNControllerManager.start_ryu_manager"
    )
    @patch(
        "csle_common.controllers.sdn_controller_manager.SDNControllerManager.get_ryu_status_by_port_and_ip"
    )
    def test_get_ryu_status(
        self, mock_get_ryu_status_by_port_and_ip, mock_get_ryu_manager
    ) -> None:
        """
        ...
        result = SDNControllerManager.get_ryu_status(
    ...
    def test_get_ryu_status_by_port_and_ip(
        self, mock_get_ryu_status, mock_insecure_channel
    ) -> None:
        """
        Test method for querying the RyuManager about the status of the Ryu SDN controller
        ...
        assert isinstance(
            mock_get_ryu_status.call_args[0][0], ryu_manager_pb2_grpc.RyuManagerStub
        )
        assert result == ryu_dto_mock
    ...
    def test_stop_ryu(
        self, mock_stop_ryu, mock_insecure_channel, mock_start_manager
    ) -> None:
        """
        ...
        ip = "192.168.1.2"
        port = 6633
        result = SDNControllerManager.stop_ryu(
            emulation_env_config=emulation_env_config, logger=logger
        )
        mock_insecure_channel.assert_called_once_with(
            f"{ip}:{port}", options=constants.GRPC_SERVERS.GRPC_OPTIONS
        ...
        assert isinstance(
            mock_stop_ryu.call_args[0][0], ryu_manager_pb2_grpc.RyuManagerStub
        )
        assert result == ryu_dto_mock
    ...
    def test_start_ryu(
        self, mock_start_ryu, mock_insecure_channel, mock_start_manager
    ) -> None:
        """
        ...
        result = SDNControllerManager.start_ryu(
            emulation_env_config=emulation_env_config,
            physical_server_ip=ip,
            logger=logger,
        )
        mock_insecure_channel.assert_called_once_with(
            "192.168.1.2:6633", options=constants.GRPC_SERVERS.GRPC_OPTIONS
        )
        mock_start_ryu.assert_called_once()
        assert isinstance(
            mock_start_ryu.call_args[0][0], ryu_manager_pb2_grpc.RyuManagerStub
        )
        assert result == ryu_dto_mock
    ...
    def test_start_ryu_monitor(
        self, mock_start_ryu_monitor, mock_insecure_channel, mock_start_manager
    ) -> None:
        """
        ...
        kafka_config = MagicMock(
            container=MagicMock(get_ips=MagicMock(return_value=["192.168.1.3"])),
            kafka_port=9092,
        )
        emulation_env_config = MagicMock(
            sdn_controller_config=sdn_controller_config,
        ...
        ip = "192.168.1.1"
        result = SDNControllerManager.start_ryu_monitor(
            emulation_env_config=emulation_env_config,
            physical_server_ip=ip,
            logger=logger,
        )
        ...
        assert isinstance(
            mock_start_ryu_monitor.call_args[0][0], ryu_manager_pb2_grpc.RyuManagerStub
        )
        assert result == ryu_dto_mock
    ...
    def test_stop_ryu_monitor(
        self, mock_stop_ryu_monitor, mock_insecure_channel, mock_start_manager
    ) -> None:
        """
        ...
        result = SDNControllerManager.stop_ryu_monitor(
            emulation_env_config=emulation_env_config, logger=logger
        )
        mock_insecure_channel.assert_called_once_with(
            "192.168.1.2:6633", options=constants.GRPC_SERVERS.GRPC_OPTIONS
        ...
        assert isinstance(
            mock_stop_ryu_monitor.call_args[0][0], ryu_manager_pb2_grpc.RyuManagerStub
        )
        assert result == ryu_dto_mock
    ...
    @patch(
        "csle_common.controllers.sdn_controller_manager.SDNControllerManager.get_ryu_managers_ips"
    )
    @patch(
        "csle_common.controllers.sdn_controller_manager.SDNControllerManager.get_ryu_managers_ports"
    )
    @patch(
        "csle_common.controllers.sdn_controller_manager.SDNControllerManager.get_ryu_status_by_port_and_ip"
    )
    ...
    @patch(
        "csle_collector.ryu_manager.ryu_manager_util.RyuManagerUtil.ryu_dto_empty",
    ...
    def test_get_ryu_managers_info(
        ...
        mock_get_ryu_status_by_port_and_ip,
        mock_get_ryu_managers_ports,
        mock_get_ryu_managers_ips,
    ) -> None:
        """
        ...
        result = SDNControllerManager.get_ryu_managers_info(
            emulation_env_config, active_ips, logger, physical_server_ip
        )
        mock_get_ryu_managers_ips.assert_called_once_with(
            emulation_env_config=emulation_env_config
        )
        mock_get_ryu_managers_ports.assert_called_once_with(
            emulation_env_config=emulation_env_config
        )
        mock_physical_ip_match.assert_called_once_with(
            emulation_env_config=emulation_env_config,
            ip="192.168.1.2",
            physical_host_ip="192.168.1.1",
        )
        mock_get_ryu_status_by_port_and_ip.assert_called_once_with(
            port=6633, ip="192.168.1.2"
        )
        assert result
