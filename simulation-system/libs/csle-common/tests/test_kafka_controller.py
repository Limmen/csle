...
class TestKafkaControllerSuite:
    ...
    def test_start_stop_kafka_manager(
    ...
    @patch(
        "csle_common.controllers.kafka_controller.KafkaController.start_kafka_manager"
    )
    @patch(
        "csle_common.controllers.kafka_controller.KafkaController.get_kafka_status_by_port_and_ip"
    )
    ...
    def test_create_topics(
        ...
        KafkaController.create_topics(
            emulation_env_config=emulation_env_config, logger=mock_logger
        )
        mock_logger.info.assert_any_call(
            "creating kafka topics on container: 127.0.0.1"
        )
        ...
        mock_start_kafka_manager.assert_called_once_with(
            emulation_env_config=emulation_env_config
        )
        mock_get_kafka_status_by_port_and_ip.assert_called_once_with(
            ip="127.0.0.1", port=9092
        )
        mock_start_kafka.assert_called_once_with(mock_stub)
        mock_create_topic.assert_any_call(
            mock_stub, name="topic1", partitions=1, replicas=1, retention_time_hours=24
        )
        mock_create_topic.assert_any_call(
            mock_stub, name="topic2", partitions=2, replicas=2, retention_time_hours=48
        )
    ...
    def test_get_kafka_status(
        self, mock_get_kafka_status_by_port_and_ip, mock_start_kafka_manager
    ) -> None:
        """
        ...
        expected_kafka_dto = MagicMock(
            spec=csle_collector.kafka_manager.kafka_manager_pb2.KafkaDTO
        )
    ...
    def test_get_kafka_status_by_port_and_ip(
        self, mock_insecure_channel, mock_KafkaManagerStub, mock_get_kafka_status
    ) -> None:
        """
    ...
    def test_stop_kafka_server(
        ...
        emulation_env_config.kafka_config.container.docker_gw_bridge_ip = "127.0.0.1"
        emulation_env_config.kafka_config.kafka_manager_port = 9092
        result = KafkaController.stop_kafka_server(
            emulation_env_config=emulation_env_config, logger=mock_logger
        )
        mock_logger.info.assert_any_call(
            "Stopping kafka server on container: 127.0.0.1"
        )
        mock_start_kafka_manager.assert_called_once_with(
            emulation_env_config=emulation_env_config
        )
    ...
    def test_start_kafka_server(
        ...
        emulation_env_config.kafka_config.container.docker_gw_bridge_ip = "127.0.0.1"
        emulation_env_config.kafka_config.kafka_manager_port = 9092
        result = KafkaController.start_kafka_server(
            emulation_env_config=emulation_env_config, logger=mock_logger
        )
        mock_logger.info.assert_any_call(
            "Starting kafka server on container: 127.0.0.1"
        )
        mock_start_kafka_manager.assert_called_once_with(
            emulation_env_config=emulation_env_config
        )
    ...
    def test_configure_broker_ips(
        self, mock_kafka_config_file, mock_connect_admin
    ) -> None:
        """
        ...
        mock_file = MagicMock()
        emulation_env_config.get_connection.return_value.open_sftp.return_value = (
            mock_sftp_client
        )
        ...
        mock_logger.info.assert_any_call(
            "Configuring broker IPs on container: 127.0.0.1"
        )
        mock_connect_admin.assert_called_once_with(
            emulation_env_config=emulation_env_config, ip="127.0.0.1"
        )
        ...
        # expected_file_contents = "192.168.1.100 127.0.0.1"
        # mock_file.write.assert_called_once_with(data=expected_file_contents)
    ...
    @patch(
        "csle_common.controllers.kafka_controller.KafkaController.get_kafka_managers_ips"
    )
    @patch(
        "csle_common.controllers.kafka_controller.KafkaController.get_kafka_managers_ports"
    )
    @patch(
        "csle_common.controllers.kafka_controller.KafkaController.get_kafka_status_by_port_and_ip"
    )
    ...
    def test_get_kafka_managers_infos(
        self,
        mock_kafka_dto_empty,
        mock_get_kafka_status_by_port_and_ip,
        mock_get_kafka_managers_ports,
        ...
        result = KafkaController.get_kafka_managers_info(
            emulation_env_config, active_ips, logger, physical_host_ip
        )
        mock_get_kafka_managers_ips.assert_called_once_with(
            emulation_env_config=emulation_env_config
        )
        mock_get_kafka_managers_ports.assert_called_once_with(
            emulation_env_config=emulation_env_config
        )
