...
class EnvState:
    ...
    def __init__(self, env_config : CSLEEnvConfig,
    ...
    def get_attacker_observation(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        if self.env_config.state_type == StateType.BASE:
        ...
        elif self.env_config.state_type == StateType.COMPACT:
            machines_obs, ports_protocols_obs = \
                AttackerStateRepresentation.compact_representation(
                    num_machines=self.attacker_obs_state.num_machines,
                    num_ports=self.attacker_obs_state.num_ports, obs_state=self.attacker_obs_state)
        elif self.env_config.state_type == StateType.ESSENTIAL:
            machines_obs, ports_protocols_obs = \
                AttackerStateRepresentation.essential_representation(
                    num_machines=self.attacker_obs_state.num_machines,
                    num_ports=self.attacker_obs_state.num_ports, obs_state=self.attacker_obs_state)
        elif self.env_config.state_type == StateType.SIMPLE:
            machines_obs, ports_protocols_obs = \
        ...
        elif self.env_config.state_type == StateType.CORE:
            machines_obs, ports_protocols_obs = \
                AttackerStateRepresentation.simple_representation(
                    num_machines=self.attacker_obs_state.num_machines,
                    num_ports=self.attacker_obs_state.num_ports, obs_state=self.attacker_obs_state)
        elif self.env_config.state_type == StateType.TEST:
            machines_obs, ports_protocols_obs = \
                AttackerStateRepresentation.test_representation(
                    num_machines=self.attacker_obs_state.num_machines,
                    num_ports=self.attacker_obs_state.num_ports,
                    obs_state=self.attacker_obs_state)
        elif self.env_config.state_type == StateType.BASIC:
            machines_obs, ports_protocols_obs = \
            ...
            raise ValueError("State type:{} not recognized".format(self.env_config.state_type))
        return machines_obs, ports_protocols_obs
        ...
        if self.env_config.state_type == StateType.BASE:
            machines_obs, network_obs =  \
                DefenderStateRepresentation.base_representation(
                    ...
                    os_lookup=self.os_lookup, ids=self.env_config.ids_router,
            ...
            machines_obs, network_obs = \
                DefenderStateRepresentation.essential_representation(
                    num_machines=self.defender_obs_state.num_machines,
                    obs_state=self.defender_obs_state, os_lookup=self.os_lookup, ids=self.env_config.ids_router,
                    multiple_stopping=self.env_config.multiple_stopping_environment, env_config=self.env_config)
        elif self.env_config.state_type == StateType.SIMPLE:
            machines_obs, network_obs = \
    ...
    def reset_state(self) -> None:
        ...
        self.attacker_obs_state = AttackerObservationState(num_machines=self.env_config.num_nodes,
                                                           num_ports=self.env_config.attacker_num_ports_obs,
                                                           num_vuln=self.env_config.attacker_num_vuln_obs,
                                                           num_sh=self.env_config.attacker_num_sh_obs,
                                                           num_flags=self.env_config.num_flags,
        ...
        if self.defender_obs_state is not None:
            ...
            self.defender_obs_state = DefenderObservationState(
                num_machines=self.env_config.num_nodes, ids=self.env_config.ids_router,
    ...
    def copy(self) -> "EnvState":
        ...
        """
        copy = EnvState(env_config=self.env_config,
    ...
    @staticmethod
    def from_emulation_env_config(emulation_env_config: EmulationEnvConfig, attacker_num_ports_obs: int=2,
                                  attacker_num_vuln_obs : int = 2, attacker_num_sh_obs : int=2,
                                  simulate_detection=False, detection_reward : int=10, base_detection_p : float = 0.01,
                                  manual_play : bool = False, state_type: StateType = StateType.BASE) -> "EnvState":
        """
        Returns a default state from an emulation environment configuration

        :param emulation_env_config: the emulation environment configuration
        :param attacker_num_ports_obs: the number of ports to include in the attacker observation
        :param attacker_num_vuln_obs: the number of vulnerabilities to include in the attacker observation
        :param attacker_num_sh_obs: the number of comromised shells to include in the attacker observation
        :param simulate_detection: whether to simulate detection or not
        :param detection_reward: the detection reward
        :param base_detection_p: the base detection reward
        :param manual_play: whether it is manual play or not
        :param state_type: the state type
        :return: the state
        """
        env_config = CSLEEnvConfig.from_emulation_config(
            emulation_env_config=emulation_env_config,
            attacker_num_ports_obs=attacker_num_ports_obs,
            attacker_num_vuln_obs = attacker_num_vuln_obs, attacker_num_sh_obs=attacker_num_sh_obs,
            simulate_detection=simulate_detection, detection_reward=detection_reward, base_detection_p = base_detection_p,
            manual_play = manual_play, state_type = state_type
        )
        s = EnvState(
            env_config=env_config, vuln_lookup=constants.VULNERABILITIES.vuln_lookup,
            service_lookup=constants.SERVICES.service_lookup, os_lookup=constants.OS.os_lookup
        )
        return s
