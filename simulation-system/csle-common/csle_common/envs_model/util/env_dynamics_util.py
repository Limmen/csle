...
class EnvDynamicsUtil:
    ...
    def merge_complete_obs_state(old_obs_state: AttackerObservationState, new_obs_state : AttackerObservationState,
                                 emulation_env_agent_config: EmulationEnvAgentConfig) -> AttackerObservationState:
        """
        ...
        net_outcome = EnvDynamicsUtil.merge_new_obs_with_old(old_obs_state.machines, new_obs_state.machines,
                                                             emulation_env_agent_config=emulation_env_agent_config, action=None)
    ...
    def merge_new_obs_with_old(old_machines_obs: List[AttackerMachineObservationState],
                               new_machines_obs: List[AttackerMachineObservationState], emulation_env_agent_config:
            EmulationEnvAgentConfig,
                               action : Union[AttackerAction, None]) -> NetworkOutcome:
        """
        ...
        for n_m in new_machines_obs:
            if emulation_env_agent_config.emulation_env_config.containers_config.agent_ip in n_m.ips:
                continue
            exists = False
            merged_m = n_m
    ...
    @staticmethod
    def is_all_flags_collected(s: EmulationEnvState, emulation_env_agent_config: EmulationEnvAgentConfig) -> bool:
        """
        ...
        """
        total_flags = set()
        for key, flag in emulation_env_agent_config.network_conf.flags_lookup.items():
    ...
    @staticmethod
    def cache_action(emulation_env_agent_config: EmulationEnvAgentConfig, a: AttackerAction, s: EmulationEnvState) -> None:
        """
        ...
        logged_in_ips_str = EnvDynamicsUtil.logged_in_ips_str(
            emulation_env_agent_config=emulation_env_agent_config, a=a, s=s)
        s.attacker_obs_state.actions_tried.add((a.id, a.index, logged_in_ips_str))

    @staticmethod
    def logged_in_ips_str(emulation_env_agent_config: EmulationEnvAgentConfig, a: AttackerAction,
        ...
        """
        hacker_ip = emulation_env_agent_config.emulation_env_config.containers_config.agent_ip
