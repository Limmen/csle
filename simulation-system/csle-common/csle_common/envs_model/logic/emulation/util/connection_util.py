...
from csle_common.dao.network.connection_setup_dto import ConnectionSetupDTO
...
class ConnectionUtil:
    ...
    def login_service_helper(s: EmulationEnvState, a: AttackerAction, alive_check, service_name: str,
                             emulation_env_agent_config: EmulationEnvAgentConfig) -> Tuple[EmulationEnvState, NetworkOutcome, bool]:
        """
        ...
        :param emulation_env_agent_config: environment config
        ...
        if target_machine is None or root or len(non_used_nor_cached_credentials) == 0:
            ...
            if target_machine is not None:
                net_outcome = EnvDynamicsUtil.merge_new_obs_with_old(
                    ...
                    emulation_env_agent_config=emulation_env_agent_config, action=a)
        ...
        target_connections = []
        ports = []
        proxy_connections = [ConnectionObservationState(conn=emulation_env_agent_config.emulation_config.agent_conn,
                                                        username=emulation_env_agent_config.emulation_config.agent_username,
                                                        root=True, port=22, service=constants.SSH.SERVICE_NAME,
                                                        proxy=None, ip=emulation_env_agent_config.emulation_config.agent_ip)]
        for m in s.attacker_obs_state.machines:
        ...
        if service_name == constants.SSH.SERVICE_NAME:
            connection_setup_dto = ConnectionUtil._ssh_setup_connection(
                a=a, emulation_env_agent_config=emulation_env_agent_config,
            ...
            connection_setup_dto = ConnectionUtil._telnet_setup_connection(
                a=a, emulation_env_agent_config=emulation_env_agent_config, credentials=non_used_nor_cached_credentials,
                proxy_connections=proxy_connections, s=s)
        elif service_name == constants.FTP.SERVICE_NAME:
                ...
                for i in range(len(connection_setup_dto.target_connections)):
                    ...
                    if service_name == constants.SSH.SERVICE_NAME:
                        c_root, cost = ConnectionUtil._ssh_finalize_connection(
                            target_machine=target_machine, i=i, emulation_env_agent_config=emulation_env_agent_config,
    ...
    @staticmethod
    def _ssh_setup_connection(a: AttackerAction, emulation_env_agent_config: EmulationEnvAgentConfig,
        ...
        :param emulation_env_agent_config: the environment config
        ...
        for proxy_conn in proxy_connections:
            if proxy_conn.ip != emulation_env_agent_config.emulation_env_config.containers_config.agent_ip:
    ...
    @staticmethod
    def _ssh_finalize_connection(target_machine: AttackerMachineObservationState, emulation_env_agent_config: EmulationEnvAgentConfig,
    ...
    @staticmethod
    def _telnet_setup_connection(a: AttackerAction, emulation_env_agent_config: EmulationEnvAgentConfig,
        ...
        start = time.time()
        for proxy_conn in proxy_connections:
            if proxy_conn.internal_ip != emulation_env_agent_config.emulation_env_config.containers_config.agent_ip:
            ...
            for cr in credentials:
                for ip in a.ips:
                    if cr.service == constants.TELNET.SERVICE_NAME:
                        try:
                            forward_port = emulation_env_agent_config.get_port_forward_port()
    ...
    @staticmethod
    def _telnet_finalize_connection(target_machine: AttackerMachineObservationState, i: int, emulation_env_agent_config: EmulationEnvAgentConfig,
    ...
    @staticmethod
    def _ftp_setup_connection(a: AttackerAction, emulation_env_agent_config: EmulationEnvAgentConfig,
                              credentials: List[Credential], proxy_connections: List,
                              s: EmulationEnvState) -> ConnectionSetupDTO:
        """
    ...
    @staticmethod
    def find_jump_host_connection(ip, s: EmulationEnvState, emulation_env_agent_config: EmulationEnvAgentConfig) \
        ...
        if ip in s.attacker_obs_state.agent_reachable:
            c = ConnectionObservationState(conn=emulation_env_agent_config.emulation_config.agent_conn,
                                           username=emulation_env_agent_config.emulation_config.agent_username,
                                           root=True, port=22,
                                           service=constants.SSH.SERVICE_NAME,
                                           proxy=None, ip=emulation_env_agent_config.emulation_config.agent_ip)
