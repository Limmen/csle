"""
An agent for the csle env that uses the DDPG algorithm from OpenAI stable baselines
"""
import time
import math

from csle_common.rendering.video.csle_ctf_monitor import CSLECTFMonitor
...
from csle_common.agents.train_agent import TrainAgent
from csle_common.agents.config.agent_config import AgentConfig
from csle_common.agents.ddpg.impl.ddpg import DDPG
from csle_common.agents.util.agent_util import AgentUtil
...
class DDPGBaselineAgent(TrainAgent):
    ...
    def __init__(self, env: BaseCSLEEnv, attacker_config: AgentConfig, eval_env: BaseCSLEEnv):
        """
        Initialize environment and hyperparameters

        :param attacker_config: the configuration
        """
        super(DDPGBaselineAgent, self).__init__(env, attacker_config, eval_env)
    ...
    def train(self) -> BaseExperimentResult:
        ...
        model = DDPG(
            policy, self.env, learning_rate=self.attacker_config.alpha, buffer_size=self.attacker_config.buffer_size,
            learning_starts=self.attacker_config.learning_starts,batch_size=self.attacker_config.batch_size,
            tau=self.attacker_config.tau,gamma=self.attacker_config.gamma,train_freq=self.attacker_config.train_freq,
        ...
        if self.attacker_config.load_path is not None:
            DDPG.load(self.attacker_config.load_path, policy, agent_config=self.attacker_config)
        ...
        if self.attacker_config.video or self.attacker_config.gifs:
            ...
            train_eval_env = CSLECTFMonitor(self.env, self.attacker_config.video_dir + "/" + time_str, force=True,
        ...
        if self.eval_env is not None:
            eval_env = CSLECTFMonitor(self.eval_env, self.attacker_config.video_dir + "/" + time_str, force=True,
                                          video_frequency=self.attacker_config.video_frequency, openai_baseline=True)
